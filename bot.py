...
PATCH_VERSION = "PATCH33_ORIGINAL_MINIMAL_FIX"
...
CUSTOM_EMOJI_IDS = {
    ...
    "absolute": "337017423906226569",
    "super_absolute": "53705591644001411305",
    "back": "5440735760208637835",
    "draw_card": "52560374474622771673",
    "universe_selected": "526042622255599405269",
    "user_name": "53638600943736424319",
    "clan": "51973884923798042229",
    "craft": "54718932182055392288",
    "shop": "5323546959061989444",
    "shop_alt": "52404911147779907309",
...
# PATCH33: разрешаем отправлять в Telegram только те custom emoji ID,
# которые пользователь сам зафиксировал как рабочие. Все остальные
# внутренние/сомнительные ID показываются обычными emoji-fallback.
# Это чинит разделы вроде «Крафт» и «Мои карты»: Telegram больше не
# отклоняет всё сообщение из-за одного битого emoji-id.
TRUSTED_CUSTOM_EMOJI_IDS = {
    "5215377245639549895",  # старт
    "6012666146648495705",  # профиль
    "5408935401442267103",  # режимы
    "5469741319330996757",  # коллекция
    "5188344996356448758",  # награды
    "5334882760735598374",  # правила
    "6057663582705814959",  # луфи-путь
    "5258112758645282249",  # драконит
    "5330236782942379682",  # фисташки
    "5467406098367521267",  # владелец
}

def valid_custom_emoji_id(emoji_id):
    value = str(emoji_id or "").strip()
    return value in TRUSTED_CUSTOM_EMOJI_IDS
...
def shop_menu():
    rows = [
        ...
        [button(text="👑 Премиум", callback_data="premium_info"), button(text="🔁 Обмен", callback_data="exchange")],
        [button(text="⚙️ Ещё", callback_data="shop_more")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def send_modes(message, user):
    ...
    await message.answer(
        ...
        f"⚔️ Арена — быстрый бой против бота.\n"
        f"🌐 Онлайн — PvP против игрока.\n"
        f"🃏 Колода — 5 бойцов и авто-сбор.\n"
        f"🎪 Ивенты — задания и редкие награды.\n"
        f"👹 Рейд — босс дня и топ урона.\n\n"
