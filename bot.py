...
SECTION_HINTS = {
    "modes": ("⚔️ Режимы", "Выбери поле боя: арена, онлайн, рейд или события. Здесь решают колода, уровень и тактика."),
    "collection": ("🃏 Коллекция", "Твои персонажи, формы, уровни и сила. Открой карту, чтобы увидеть роль, плюс, минус и потенциал."),
    "shop": ("🎁 Награды", "Сундуки, ежедневные награды, мультипасс и редкие покупки."),
    "profile": ("👤 Профиль", "Твой ранг, ресурсы, карты, друзья и настройки."),
    "rules": ("📜 Правила", "Короткие правила мультивселенной без скучной стены текста."),
    "luffy": ("🔥 Путь Луфи", "10 дней — 10 форм Луфи. Забирай путь постепенно, без спама."),
    "newbie": ("🔥 Путь Луфи", "10 дней — 10 форм Луфи. Забирай путь постепенно, без спама."),
}
...
def main_menu(user_id=None):
    rows = [
        [InlineKeyboardButton(text="⚔️ Режимы", callback_data="modes"), InlineKeyboardButton(text="🃏 Коллекция", callback_data="collection:page:0")],
        [InlineKeyboardButton(text="🎁 Награды", callback_data="shop"), InlineKeyboardButton(text="👤 Профиль", callback_data="profile")],
        [InlineKeyboardButton(text="📜 Правила", callback_data="rules")],
    ]
    if user_id:
        player = DATA.get("users", {}).get(str(user_id), {})
        if not player.get("luffy_finished") and int(player.get("luffy_day", 0) or 0) < len(LUFFY_PATH_CARDS):
            rows.insert(2, [InlineKeyboardButton(text="🔥 Путь Луфи", callback_data="luffy_path")])
...
def main_menu_text(user=None):
    return (
        ...
        "Выбери свой путь.\n\n"
        "Собирай персонажей, усиливай колоду и сражайся в режимах мультивселенной."
    )
async def maybe_send_luffy_intro(message, user, force=False):
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔥 Открыть Луфи", callback_data="luffy_path")],
...
def shop_menu():
    rows = [
        ...
        [InlineKeyboardButton(text="🧰 Сундуки", callback_data="chests"), InlineKeyboardButton(text="🎟 Мультипасс / Донат", callback_data="donate_menu")],
...
async def send_profile(message, user):
    ...
    await message.answer(
        ...
        f"🐉 Драконит: <b>{p.get('moon_coins', 0)}</b>\n"
...
async def send_rules(message):
    await message.answer(
        ...
        "🎴 <b>Редкости</b>\n"
        "⚪ Origin — базовые формы.\n"
        "🔷 Rare — усиленные формы.\n"
        "🟣 Epic — сильные режимы и техники.\n"
        "🟡 Legendary — пиковые версии персонажей.\n"
        "🔴 Absolute — самые опасные формы мультивселенной.\n\n"
        "💎 <b>Фисташки</b>\n"
        ...
        "📩 <b>Проблема</b>\n"
        "Пиши владельцу и прикладывай скрин: профиль, бой, время и что именно сломалось. Покемоны учитываются по всей франшизе, не только по фильмам.",
        reply_markup=back_menu(),
        parse_mode="HTML"
    )
...
async def send_shop(message, user):
    ...
    await message.answer(
        ...
        f"🐉 Драконит: <b>{p.get('moon_coins', 0)}</b>\n\n"
        "Качай персонажей, открывай сундуки и забирай награды без лишних вкладок.",
...
async def donate_menu_cb(callback: types.CallbackQuery):
    rows = [
        ...
        [InlineKeyboardButton(text="🧰 Кейсы", callback_data="cases"), InlineKeyboardButton(text="🏷 Знаки", callback_data="badges_shop")],
        [InlineKeyboardButton(text="🎴 Мега-открытие", callback_data="mega_open"), InlineKeyboardButton(text="🎟 Промокод", callback_data="promo_help")],
        [InlineKeyboardButton(text="💳 Купить уровень", callback_data="buy_pass_level")],
        [InlineKeyboardButton(text="⬅️ Магазин / награды", callback_data="shop")],
    ]
...
async def send_collection(message, user, page=0, rarity_filter="all", sort_mode="power"):
    ...
    rows.append([
        ...
        InlineKeyboardButton(text="⚪ Origin", callback_data="collection:filter:common"),
        InlineKeyboardButton(text="🔷 Rare", callback_data="collection:filter:rare"),
    ])
    rows.append([
        InlineKeyboardButton(text="🟣 Epic", callback_data="collection:filter:epic"),
        InlineKeyboardButton(text="🟡 Legendary", callback_data="collection:filter:legendary"),
        InlineKeyboardButton(text="🔴 Absolute", callback_data="collection:filter:mythic"),
    ])
    rows.append([
...
async def card_detail(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"📖 <b>Описание:</b> {e(c.get('description', 'Описание скоро будет обновлено.'))}\n\n"
    ...
    rows.append([InlineKeyboardButton(text="🎴 В команду", callback_data=f"deck_add:{cid}"), InlineKeyboardButton(text="⬅️ Назад", callback_data="collection:page:0:all:power")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def deck_add_cb(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🧬 Открыть колоду", callback_data="deck")],
            [InlineKeyboardButton(text="⬅️ Коллекция", callback_data="collection:page:0:all:power")],
        ]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def show_difficulty_select(message, user, arena_code):
    ...
    rows.append([InlineKeyboardButton(text="⬅️ Арены", callback_data="battle:arena_select")])
    text = (
...
async def send_battle_source_menu(message, user, target="solo"):
    ...
    rows = [
        [InlineKeyboardButton(text="🛡️ Своя колода", callback_data=f"battle_source:{target}:deck")],
        [InlineKeyboardButton(text="🤖 Авто-колода от бота", callback_data=f"battle_source:{target}:random_bot")],
        [InlineKeyboardButton(text="🎴 Выбрать карты вручную", callback_data=f"battle_source:{target}:manual")],
        [InlineKeyboardButton(text="⬅️ Режимы", callback_data="modes")],
    ]
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def send_manual_team_picker(message, user, target="solo", page=0):
    ...
    owned.sort(key=lambda x: card_power(CARD_BY_ID[x[0]], int(x[1].get("level", 1))), reverse=True)
    if not owned:
        await message.answer("🎴 В коллекции нет карт. Для боя будет использована временная авто-колода.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Продолжить", callback_data=f"manual_team_done:{target}")]]))
        return
    per_page = 7
    pages = max(1, (len(owned) + per_page - 1) // per_page)
    ...
    for cid, info in owned[page*per_page:(page+1)*per_page]:
        ...
        rows.append([InlineKeyboardButton(text=f"{mark} {c['name'][:30]}", callback_data=f"manual_team_add:{target}:{cid}")])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"manual_team_page:{target}:{page-1}"))
    if page < pages - 1:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"manual_team_page:{target}:{page+1}"))
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text="✅ Готово", callback_data=f"manual_team_done:{target}"), InlineKeyboardButton(text="🧹 Сброс", callback_data=f"manual_team_clear:{target}")])
    rows.append([InlineKeyboardButton(text="🤖 Авто-колода", callback_data=f"battle_source:{target}:random_bot")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def battle_source_cb(callback: types.CallbackQuery):
    ...
    if source == "manual":
        ...
        await callback.message.answer("✅ Тип команды сохранён для PvP/рейда.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🌐 Онлайн", callback_data="online_search")], [InlineKeyboardButton(text="👹 Рейд", callback_data="raid_info")], [InlineKeyboardButton(text="⬅️ Режимы", callback_data="modes")]]), parse_mode="HTML")
    await callback.answer()
...
@dp.callback_query(F.data.startswith("manual_team_add:"))
async def manual_team_add_cb(callback: types.CallbackQuery):
    try:
        _, _, target, cid = callback.data.split(":", 3)
    except Exception:
...
async def manual_team_done_cb(callback: types.CallbackQuery):
    ...
    if target == "solo":
        ...
        await callback.message.answer("✅ Ручная команда сохранена.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🌐 Онлайн", callback_data="online_search")], [InlineKeyboardButton(text="👹 Рейд", callback_data="raid_info")], [InlineKeyboardButton(text="⬅️ Режимы", callback_data="modes")]]), parse_mode="HTML")
    await callback.answer()
...
async def raid_info_cb(callback: types.CallbackQuery):
    ...
    rows = [[InlineKeyboardButton(text="⚔️ Ударить босса", callback_data="raid_hit")]]
    if is_owner(callback.from_user.id):
        rows.append([
            InlineKeyboardButton(text="👑 Админ-удар", callback_data="admin_raid_hit"),
            InlineKeyboardButton(text="☠️ Добить босса", callback_data="admin_raid_kill"),
        ])
    rows.append([InlineKeyboardButton(text="⬅️ Ивенты", callback_data="events")])
...
async def raid_hit_cb(callback: types.CallbackQuery):
    ...
    if last:
        try:
            ...
            pass
    team = build_team_for_user(callback.from_user.id, source=p.get("battle_team_source", p.get("pvp_team_source", "deck")), fill=True)
    if len(team) < 5:
...
async def admin_raid_hit_cb(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🐉 Рейд", callback_data="raid_info")]]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def admin_raid_kill_cb(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏯 Ивенты", callback_data="events")]]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def send_newbie_start(message, user):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔥 Путь Луфи 10 дней", callback_data="luffy_path")],
...
async def send_luffy_path(message, user):
    ...
    rows = []
    if ready:
        rows.append([InlineKeyboardButton(text="🎁 Забрать форму дня", callback_data="luffy_claim")])
    rows.append([InlineKeyboardButton(text="🎁 Daily", callback_data="daily"), InlineKeyboardButton(text="🆓 Сундук", callback_data="pack_info:free")])
    rows.append([InlineKeyboardButton(text="⬅️ Меню", callback_data="menu")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def luffy_claim_cb(callback: types.CallbackQuery):
    ...
    c = CARD_BY_ID[cid]
    await callback.message.answer(
        f"🔥 <b>Путь Луфи — день {day + 1}/10</b>\n\n"
        f"{rarity_label(c['rarity'])} <b>{e(c['name'])}</b>\n"
        f"Форма: <b>{e(c.get('form',''))}</b>\n"
        f"{e(result)}\n"
        "+150 💎 и +80 очков pass.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔥 Путь Луфи", callback_data="luffy_path")],
            [InlineKeyboardButton(text="⬅️ Меню", callback_data="menu")],
        ]),
        parse_mode="HTML"
    )
...
async def send_multipass(message, user):
    ...
    rows = [
        [InlineKeyboardButton(text="🎯 Задания", callback_data="pass_tasks")],
        [InlineKeyboardButton(text="🎁 Бесплатные награды", callback_data="pass_claim:free"), InlineKeyboardButton(text="👑 Premium награды", callback_data="pass_claim:premium")],
        [InlineKeyboardButton(text="💳 Купить уровень", callback_data="buy_pass_level"), InlineKeyboardButton(text="⭐ Stars-наборы", callback_data="stars_shop")],
    ]
    if not p.get("pass_premium") and p.get("pass_purchase_request") != "paid_pending" and not is_owner(user.id):
        rows.append([InlineKeyboardButton(text=f"⭐ Купить Premium за {PASS_PRICE_STARS} Stars", callback_data="buy_pass_stars")])
    rows.append([InlineKeyboardButton(text="⬅️ Меню", callback_data="menu")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def buy_pass_level_cb(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Мультипасс", callback_data="multipass")]]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def send_pass_tasks(message, user):
    ...
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Забрать очки", callback_data="pass_claim_tasks")],
        [InlineKeyboardButton(text="⬅️ Мультипасс", callback_data="multipass")],
    ]), parse_mode="HTML")
...
async def send_modes(message, user):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚔️ Арена", callback_data="battle:start"), InlineKeyboardButton(text="🌐 Онлайн", callback_data="online_search")],
        [InlineKeyboardButton(text="🃏 Колоды", callback_data="deck"), InlineKeyboardButton(text="🎪 Ивенты", callback_data="events")],
        [InlineKeyboardButton(text="👹 Рейд", callback_data="raid_info"), InlineKeyboardButton(text="⚙️ Выбор боя", callback_data="pvp_source_menu")],
        [InlineKeyboardButton(text="⬅️ Меню", callback_data="menu")],
    ])
    title, body = SECTION_HINTS["modes"]
    await message.answer(
        f"{ui_box(title, body)}\n\n"
        "⚔️ <b>Арена</b> — бой против ИИ на поле с плюсами и минусами.\n"
        "🌐 <b>Онлайн</b> — PvP против игрока.\n"
        "🃏 <b>Колоды</b> — настрой свою команду.\n"
        "🎪 <b>Ивенты</b> — события и временные активности.\n"
        "👹 <b>Рейд</b> — общий босс, рейтинг урона и награды.\n\n"
        f"⚙️ Выбор боя: <b>{e(source_names.get(p.get('pvp_team_source', 'deck'), 'моя колода'))}</b>.",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def pvp_source_legacy_cb(callback: types.CallbackQuery):
    ...
    if source == "manual":
        ...
        await callback.message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🌐 Онлайн", callback_data="online_search")],
                [InlineKeyboardButton(text="👹 Рейд", callback_data="raid_info")],
                [InlineKeyboardButton(text="⬅️ Режимы", callback_data="modes")],
            ]),
            parse_mode="HTML",
        )
...
async def send_admin_panel(message, user):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 Компенсация", callback_data="admin_compensation_info"), InlineKeyboardButton(text="🧠 Хранилище", callback_data="admin_storage")],
        [InlineKeyboardButton(text="⬅️ Меню", callback_data="menu")],
    ])
    save_json(DATA_FILE, DATA)
    await message.answer(text, reply_markup=kb, parse_mode="HTML")
...
async def send_admin_users(message, page=0):
    ...
    for i, (uid, p) in enumerate(chunk, page * per_page + 1):
        ...
        rows.append([InlineKeyboardButton(text=f"📊 {display}", callback_data=f"admin_user:{uid}")])
    nav = []
    if page > 0:
    ...
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text="🔄 Обновить", callback_data=f"admin_users:{page}"), InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="admin")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def send_admin_user(message, uid):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🆔 Показать ID", callback_data=f"admin_show_id:{uid}")],
...
async def admin_compensation_info_cb(callback: types.CallbackQuery):
    if not is_owner(callback.from_user.id):
        ...
        return
    await callback.message.answer(
        "🎁 <b>Компенсация PATCH15</b>\n\n"
        "Разовая команда владельца:\n"
        "<code>/compensate_patch15</code>\n\n"
        f"Выдаёт каждому игроку один раз: +{COMPENSATION_FISTIKS} 💎 +{COMPENSATION_MOON_COINS} 🐉 +{COMPENSATION_PASS_XP} pass XP и отправляет сообщение с извинением. Owner пропускается для чистой статистики.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="admin")]]),
        parse_mode="HTML"
    )
    await callback.answer()
