...
PATCH_VERSION = "PATCH34_EMOJI_UI_COLLECTION_FIX"
...
def card_public_description(card):
    """Красивое описание без мусорных PATCH/тренировочных заглушек."""
    card = card or {}
    raw = str(card.get("description", "") or "").strip()
    bad_markers = ("Карта добавлена", "PATCH29", "PATCH30", "World System", "Тренировочный режим")
    if raw and not any(marker in raw for marker in bad_markers) and len(raw) >= 80:
        return raw
    name = _clean_card_field(card.get("name"), "Персонаж")
    universe = _clean_card_field(card.get("universe_name") or card.get("anime"), "мультивселенной")
    form = _clean_card_field(card.get("form"), "боевая форма")
    role = _clean_card_field(card.get("role"), "универсальный боец")
    plus = _clean_card_field(card.get("plus"), "стабильно усиливает команду")
    minus = _clean_card_field(card.get("minus"), "требует грамотной поддержки")
    abilities = _clean_card_field(card.get("abilities"), "тактическое давление и командная синергия")
    rarity = rarity_label_for_card(card)
    return (
        f"{name} — карта вселенной «{universe}». В этой версии персонаж раскрыт как «{form}»: "
        f"его задача в отряде — {role}. Редкость карты: {rarity}. "
        f"Сильная сторона: {plus}. Слабое место: {minus}. "
        f"Ключевые возможности: {abilities}. Лучше всего показывает себя в колоде, где есть поддержка, контроль темпа и усиление выбранной вселенной."
    )
...
async def ensure_quick_keyboard(message, user):
    # PATCH30: автоматическое сообщение "быстрые кнопки включены" убрано.
    # Telegram не умеет показать reply-клавиатуру совсем без исходящего сообщения,
    # поэтому быстрые кнопки обновляются через /keyboard или обычные ответы без спама.
    get_user_data(user)
    return
...
async def send_universe_menu(message, user, page=0, intro=False):
    ...
    text = (
        ...
        f"{CE['universe_selected']} Сейчас: <b>{e(universe_label(current))}</b>"
    )
    rows = []
    all_mark = "✅" if current == "all" else "🌌"
    rows.append([button(text=f"{all_mark} Мультивселенная", callback_data="universe:set:all")])
    for rec in shown:
        uid = rec["id"]
        mark = "✅" if uid == current else universe_emoji(uid)
        rows.append([button(text=f"{mark} {rec['name'][:34]}", callback_data=f"universe:set:{uid}")])
    nav = []
    if page > 0:
...
async def universe_set_cb(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"{CE['universe_selected']} Вы выбрали: <b>{e(label)}</b>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [button(text="🎴 Получить карту", callback_data="draw_card"), button(text="🃏 Мои карты", callback_data="collection:home")],
...
async def send_shop(message, user):
    ...
    await message.answer(
        ...
        "Здесь лежит всё для ускорения прогресса: попытки, кейсы, MultiPass, премиум и обмен. Выбери нужный раздел ниже.",
...
async def send_collection_home(message, user):
    ...
    text = (
        ...
        "Выбери тип карт для просмотра коллекции.\n"
        "Поиск конкретной карты: <code>/findcard имя</code>\n\n"
        f"<blockquote>"
    ...
    rows = [
        [button(text="📚 Обычные карты", callback_data="collection:page:0:all:power"), button(text="🎪 Ивентовые", callback_data="collection:filter:event")],
        [button(text="🔴 Absolute", callback_data="collection:filter:mythic"), button(text="⚫ Super Absolute", callback_data="collection:filter:super")],
        [button(text="🧿 Мистик", callback_data="collection:filter:mystic"), button(text="🧿 Артефакты", callback_data="artifacts:page:0")],
        [button(text="🧩 Фрагменты", callback_data="fragments:page:0:all")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def send_collection(message, user, page=0, rarity_filter="all", sort_mode="power"):
    ...
    frag_count = len(scoped_fragment_card_items(p, scope_uid))
    ...
    if not items:
        ...
        await message.answer(
            ...
            f"Фрагментов к сборке: <b>{short_number(frag_count)}</b>\n"
    ...
    text = (
        ...
        f"Открыто: <b>{short_number(scoped_opened_count)}/{short_number(scoped_total_count)}</b> · фрагментов: <b>{short_number(frag_count)}</b> · артефактов: <b>{short_number(art_count)}</b>\n\n"
        + "\n".join(text_cards) +
...
async def draw_card_to_message(message, user, callback=None):
    ...
    await message.answer(
        ...
        f"{CE['collection']} <b>{e(card['name'])}</b> — {rarity_label_for_card(card)}{special}\n"
        f"{CE['universe_selected']} {e(card.get('universe_name') or card.get('anime', ''))}\n"
        f"🎭 {e(card.get('form',''))}\n"
        f"{e(result)}\n\n"
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [button(text="🃏 Мои карты", callback_data="collection:home"), button(text="⬅️ Меню", callback_data="menu")],
        ]),
        parse_mode="HTML"
    )
    if callback:
