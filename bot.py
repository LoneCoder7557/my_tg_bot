...
SECTION_HINTS = {
    ...
    "rules": (f"{CE['rules']} Правила", "Карта мультивселенной: карты, валюты, режимы, запреты и полный список аниме-источников."),
...
def anime_source_chunks(limit=3400):
    """Возвращает компактный полный список франшиз из cards.json, без риска превысить лимит Telegram."""
    names = sorted({str(c.get("anime", "")).strip() for c in CARDS if str(c.get("anime", "")).strip()}, key=str.casefold)
    if not names:
        return ["<i>Список источников пока пуст: cards.json не содержит поле anime.</i>"]
    chunks = []
    header = f"{CE['collection']} <b>Аниме-источники в боте</b>\n"
    header += "Список собран прямо из <code>cards.json</code>. Покемоны считаются по всей франшизе, не только по фильмам.\n\n"
    cur = header
    for i, name in enumerate(names, 1):
        line = f"{i}. {e(name)}\n"
        if len(cur) + len(line) > limit and cur != header:
...
async def send_rules(message):
    main_text = (
        ...
        "📩 <b>Проблема</b>\n"
        "Пиши владельцу и прикладывай скрин: профиль, бой, время и что именно сломалось."
    )
