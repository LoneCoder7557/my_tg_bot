...
PATCH_VERSION = "PATCH20_SPEED_CLEAN"
...
def random_starter_card_ids(count=STARTER_CARD_COUNT):
    pool = [c["id"] for c in CARDS if c.get("rarity") == "Обычный" and c.get("id") in CARD_BY_ID]
    if len(pool) < count:
        pool = [c["id"] for c in CARDS if c.get("id") in CARD_BY_ID]
    random.shuffle(pool)
...
async def send_admin_panel(message, user):
    ...
    live_items = active_player_items(include_blocked=False)
...
async def send_admin_users(message, page=0):
    ...
    text = (
        ...
        f"Показываются все записи из базы: онлайн, офлайн, без username, заблокированные, удалённые и старые. "
        f"Восстановлено из источников: <b>{recovery.get('sources', 0)}</b>.\n"
...
PATCH18_COMPENSATION_MOON_COINS = 10
