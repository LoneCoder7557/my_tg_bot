...
PATCH_VERSION = "PATCH23_ULTRA_RPG"
...
BOT_SHORT_DESCRIPTION = "Anime Battle Multiverse — аниме RPG-карты: вселенные, попытки, кланы, арены, рейды и коллекции."
BOT_PUBLIC_DESCRIPTION = (
    "🌌 Anime Battle Multiverse\n"
    "Анимешная RPG-мультивселенная: выбери мир, получай карту раз в 3 часа, собирай коллекцию, вступай в кланы, усиливай колоду и побеждай на арене.\n\n"
...
def record_user_action(user, action, player=None):
    ...
    try:
        ...
        # Не забиваем историю одинаковыми кнопками подряд.
        if not actions or actions[-1].get("action") != item["action"]:
            actions.append(item)
        if len(actions) > 30:
            del actions[:-30]
        mark_data_dirty("user_action")
    except Exception as ex:
...
DATA_SAVE_DEBOUNCE_SECONDS = float(os.getenv("DATA_SAVE_DEBOUNCE_SECONDS", "4"))
_DATA_DIRTY = False
...
async def _delayed_data_save():
    ...
    while True:
        ...
        _DATA_DIRTY = False
        try:
            # deepcopy делаем в event loop один раз на пачку изменений, а тяжёлые
            # JSON/SQLite/Neon операции отправляем в thread, чтобы не морозить бота.
            snapshot = copy.deepcopy(DATA)
            await asyncio.to_thread(save_json, DATA_FILE, snapshot)
...
def load_data_storage(default):
    candidates = []
    candidates.extend(list(_data_postgres_candidates()))
...
UNIVERSE_ONBOARDING_VERSION = "patch23_ultra_rpg_universes"
...
def is_super_absolute_card(card):
    s = " ".join(str(card.get(k, "")) for k in ["id", "name", "anime", "form", "role", "abilities", "description"]).casefold()
    if any(term in s for term in SUPER_ABSOLUTE_TERMS):
        return True
    anime = str(card.get("anime", "")).casefold()
    if any(term in anime for term in SUPER_ABSOLUTE_ANIME_TERMS) and card.get("rarity") in ("Легендарный", "Мифический"):
        return True
    return bool(card.get("premium_only") or card.get("super_absolute"))
        ...
        try:
            if isinstance(event, types.CallbackQuery) and event.message:
                ...
                delete_exact = {
                    ...
                    "battle:start", "online_search", "cases", "events", "universe", "draw_card", "clan", "referral",
                }
                delete_prefixes = (
                    "pack_info:", "collection:page:", "card:", "battle:arena:", "battle:arena_page:", "battle:diff:",
                    "pvp_source:", "universe:page:", "universe:set:", "shop_", "buy_attempts:", "buy_fistiks:", "buy_case_item:", "buy_battlepass:", "buy_privilege:", "clan_", "friend_profile:", "friend_gift:", "friend_sendgift:", "friend_remove:", "craft_attempts:",
                )
...
def recover_users_from_all_sources(save=True):
    """Подтягивает всех пользователей из найденных JSON/DB/backup-файлов.

    Нужна для админки: если после старых кривых патчей часть игроков оказалась
    только в backup/DB, владелец снова увидит их в «Все игроки».
    """
    DATA.setdefault("users", {})
    ...
    for _path, candidate in _iter_extra_storage_candidates():
        source_count += 1
        merged = _merge_users_data(merged, candidate)
    DATA.update(merged)
    DATA.setdefault("users", {})
    added = len(DATA["users"]) - len(before_users)
    changed = DATA.get("users", {}) != before_users
    if save and changed:
        mark_data_dirty("data_changed")
    return {"sources": source_count, "before": len(before_users), "after": len(DATA.get("users", {})), "added": max(0, added), "changed": changed}
...
def get_user_data(user):
    ...
    if uid not in DATA.setdefault("users", {}):
        ...
        DATA["users"][uid]["starter_cards"] = starter_ids
    ...
    if is_owner(user.id):
        ...
        # PATCH18: владелец должен видеть не пустую вкладку артефактов, а полный инвентарь.
        player.setdefault("artifacts", {})
        for aid, art in ARTIFACT_BY_ID.items():
            player["artifacts"][aid] = {
                "count": max(1, int(player["artifacts"].get(aid, {}).get("count", 0) or 0)),
                "level": MAX_LEVEL,
...
def main_menu(user_id=None):
    rows = [
        [button(text="🎴 Получить карту", callback_data="draw_card"), button(text="🌌 Вселенная", callback_data="universe")],
        [button(text="🏆 Рейтинг", callback_data="rating"), button(text="⚔️ Арена", callback_data="modes")],
        [button(text="⚒️ Крафт", callback_data="craft"), button(text="🏰 Клан", callback_data="clan")],
        [button(text="🧰 Кейсы", callback_data="cases"), button(text="🏪 Магазин", callback_data="shop")],
        [button(text="🔗 Рефералка", callback_data="referral"), button(text="👤 Профиль", callback_data="profile")],
...
def main_menu_text(user=None):
    ...
    if user:
        ...
        ensure_rpg_fields(p)
        name = p.get("name") or user.full_name or "Игрок"
        universe = universe_label(selected_universe_id(p))
    ...
    return (
        ...
        f"<i>{e(name)}, выбери вселенную, собирай карты и веди свой отряд к победе.</i>\n\n"
        f"🌌 <b>Вселенная:</b> {e(universe)}\n"
        f"🏰 <b>Клан:</b> {e(clan)}\n"
        f"🎴 <b>Попытки:</b> {attempts}\n"
        f"{PISTACHIOS_LABEL}: <b>{fistiks}</b>\n"
        f"{DRAGONITE_LABEL}: <b>{dragonite}</b>\n"
        f"⚔️ <b>Боевая мощь:</b> {power}\n\n"
        "🎴 Получай карту раз в 3 часа или покупай попытки.\n"
        "🌐 Онлайн-арена доступна, но пока игроков мало — быстрые бои временно идут против бота."
    )
...
PATCH23_COMPENSATION_KEY = "patch23_ultra_rpg_hub_2026_05_08"
PATCH23_COMPENSATION_FISTIKS = 3500
PATCH23_COMPENSATION_DRAGONITE = 1
PATCH23_COMPENSATION_ATTEMPTS = 7
PATCH23_COMPENSATION_PASS_XP = 900
...
PRIVILEGES = {
    ...
    "mvp": {"title": "MVP", "icon": "🔥", "cost": 599, "wait_minutes": 160, "boost": "x2.5", "attempts": 25, "fistiks": 25000},
    "x": {"title": "X", "icon": "❌", "cost": 999, "wait_minutes": 120, "boost": "x3", "attempts": 45, "fistiks": 45000},
    "op": {"title": "OP", "icon": "⚡", "cost": 2999, "wait_minutes": 105, "boost": "x4", "attempts": 70, "fistiks": 225000},
    "gold": {"title": "GOLD", "icon": "👑", "cost": 4999, "wait_minutes": 90, "boost": "x5", "attempts": 100, "fistiks": 450000},
}
...
def public_profile_text(uid, player):
    ...
    return (
        ...
        f"Боевая мощь: <b>{battle_power_label(player)}</b>\n"
...
def profile_menu(user_id=None):
    rows = [
        ...
        [button(text="🌌 Сменить вселенную", callback_data="universe"), button(text="🎨 Кастомизация", callback_data="customize")],
...
def shop_menu():
    rows = [
        ...
        [button(text="💎 Фисташки", callback_data="shop_fistiks"), button(text="🧰 Кейсы", callback_data="shop_cases")],
        [button(text="🎟 Боевой пропуск", callback_data="shop_battlepass"), button(text="👑 Премиум", callback_data="premium_info")],
        [button(text="💠 Привилегии", callback_data="privileges"), button(text="🔁 Обмен", callback_data="exchange")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def set_commands():
    ...
    owner_commands = public_commands + [
        ...
        BotCommand(command="compensate_patch23", description="Компенсация текущего обновления"),
...
async def start(message: types.Message):
    ...
    p = DATA.get("users", {}).get(str(message.from_user.id), {})
...
async def send_profile(message, user):
    ...
    pass_state = "активен" if p.get("pass_premium") or p.get("premium") else "неактивен"
    wait = free_card_wait_minutes(p)
    next_attempt = "сейчас доступна" if wait <= 0 else f"через {format_wait_hms(wait)}"
    await message.answer(
        f"{CE['profile']} <b>Профиль игрока</b>\n\n"
        f"Имя: <b>{e(p.get('name', user.full_name))}</b>\n"
        f"ID: <code>{user.id}</code>\n"
        f"Титул: <b>{e(player_title(p))}</b>\n"
        f"Роль: {role}\n"
        f"Вселенная: <b>{e(universe_label(selected_universe_id(p)))}</b>\n"
        f"Клан: <b>{e(clan_name_for(p))}</b>\n"
        f"Коллекция: <b>{unique}/{len(CARDS)}</b> · всего <b>{total}</b>\n"
        f"Боевая мощь: <b>{battle_power_label(p)}</b>\n"
        f"🎴 Попытки: <b>{available_attempts(p)}</b> · следующая бесплатная {e(next_attempt)}\n"
        f"{PISTACHIOS_LABEL}: <b>{p.get('fistiks', 0)}</b>\n"
        f"{DRAGONITE_LABEL}: <b>{p.get('moon_coins', 0)}</b>\n"
        f"Премиум: <b>{pass_state}</b>\n"
...
async def send_rules(message):
    text = (
        f"{CE['rules']} <b>Правила Anime Battle Multiverse</b>\n\n"
        "<b>1. Старт</b>\n"
        "Выбери вселенную. От неё зависит основной пул карт при получении, крафте и кейсах. Можно выбрать и режим «Все аниме».\n\n"
        "<b>2. Получение карт</b>\n"
        "Каждые 3 часа доступна бесплатная попытка. Дополнительные попытки можно получить через магазин, крафт, подарки, события и компенсации.\n\n"
        "<b>3. Валюты</b>\n"
        f"{PISTACHIOS_LABEL} — основная валюта для обычного прогресса.\n"
        f"{DRAGONITE_LABEL} — редкая премиум-валюта для кейсов, привилегий и особых покупок.\n\n"
        "<b>4. Редкости</b>\n"
        f"{CE['origin']} Origin → {CE['rare']} Rare → {CE['epic']} Epic → {CE['legendary']} Legendary → {CE['absolute']} Absolute.\n"
        "Прокачанная редкая карта может быть полезнее слабой легендарной: решают уровень, роль, HP, артефакты и команда.\n\n"
        "<b>5. Бои</b>\n"
        "Пока реальных игроков мало, быстрые бои чаще идут против бота. Онлайн-режим остаётся доступным, но может ждать соперника.\n\n"
        "<b>6. Кланы</b>\n"
        "В клане собираются игроки, растёт общий рейтинг и открываются будущие клановые события. Создание клана доступно владельцу, premium/VIP и привилегиям.\n\n"
        "<b>7. Честность</b>\n"
        "Запрещены абузы, попытки ломать оплату/прогресс, спам владельцу и выдача себя за администратора.\n\n"
        "<b>8. Споры</b>\n"
        "Если бой или награда выглядят неправильно — используй <code>/appeal причина</code>."
    )
    await message.answer(text, reply_markup=back_menu(), parse_mode="HTML")
...
async def send_universe_menu(message, user, page=0, intro=False):
    ...
    title = "🌌 <b>Выбор вселенной</b>" if not intro else "🌌 <b>Новый разлом открыт</b>"
    text = (
        f"{title}\n\n"
        f"{e(p.get('name', user.full_name))}, выбери мир для призывов.\n"
        "Коллекционируй карты, усиливай колоду и забирай победы.\n\n"
        f"Сейчас: <b>{e(universe_label(current))}</b>"
    )
    rows = []
    all_mark = "✅" if current == "all" else "🌌"
    rows.append([button(text=f"{all_mark} Все аниме", callback_data="universe:set:all")])
    pair = []
    for rec in shown:
        uid = rec["id"]
        mark = "✅" if uid == current else universe_emoji(uid)
        pair.append(button(text=f"{mark} {rec['name'][:26]}", callback_data=f"universe:set:{uid}"))
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:
...
async def universe_set_cb(callback: types.CallbackQuery):
    ...
    mark_data_dirty("data_changed")
    await callback.answer(f"Выбрано: {universe_label(uid)}")
    await send_universe_menu(callback.message, callback.from_user, 0)
...
async def send_shop(message, user):
    ...
    ensure_rpg_fields(p)
    await message.answer(
        f"🏪 <b>Магазин мультивселенной</b>\n\n"
        f"{PISTACHIOS_LABEL}: <b>{p.get('fistiks', 0)}</b>\n"
        f"{DRAGONITE_LABEL}: <b>{p.get('moon_coins', 0)}</b>\n"
        f"🎴 Попытки: <b>{available_attempts(p)}</b>\n\n"
        "Выбери раздел ниже. Главное: фисташки — обычный прогресс, драгонит — редкая премиум-валюта.",
...
async def pack_info(callback: types.CallbackQuery):
    ...
    if kind == "free":
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [button(text="🎴 Получить карту", callback_data="draw_card")],
...
async def send_craft(message, user):
    ...
    text = (
        f"⚒️ <b>Крафт</b>\n\n"
        f"{e(p.get('name', user.full_name))}, здесь дубликаты и фрагменты превращаются в новые попытки или карты.\n\n"
        "<b>Ваши фрагменты:</b>\n"
    )
    for code, rarity in RARITY_CODES.items():
        have = rarity_shards(p, rarity)
        cost = CRAFT_COSTS[rarity]
        text += f"{rarity_label(rarity)}: <b>{have}</b> / {cost}\n"
    text += (
        "\n<b>Крафт попыток:</b>\n"
        "10 Origin → 1 попытка\n"
        "10 Rare → 3 попытки\n"
        "10 Epic → 6 попыток\n"
        "10 любых фрагментов → 1 попытка\n"
    )
    rows = [
        [button(text="🎴 10 Origin → 1", callback_data="craft_attempts:common"), button(text="🎴 10 Rare → 3", callback_data="craft_attempts:rare")],
        [button(text="🎴 10 Epic → 6", callback_data="craft_attempts:epic"), button(text="⚒️ Craft всё", callback_data="craft_attempts:all")],
...
async def send_rating(message):
    ...
    def name_of(item): return item[1].get("name", item[0])
    top_power = sorted(users, key=lambda x: battle_power_label(x[1]), reverse=True)[:10]
    top_fistiks = sorted(users_money, key=lambda x: x[1].get("fistiks", 0), reverse=True)[:10]
    top_arena = sorted(users, key=lambda x: x[1].get("wins", 0), reverse=True)[:10]
    top_multi = sorted(users, key=lambda x: x[1].get("wins", 0) * 10 + len(x[1].get("collection", {})) * 3 + x[1].get("xp", 0) // 50, reverse=True)[:10]
    clans = sorted(clan_store().values(), key=lambda c: int(c.get("points",0) or 0) + len(c.get("members",[]))*100, reverse=True)[:10]
    def block(title, arr, value_fn):
        s = f"\n<b>{title}</b>\n"
        if not arr: return s + "пока пусто\n"
        for i, item in enumerate(arr, 1): s += f"{i}. {e(name_of(item))} — {value_fn(item)}\n"
        return s
    text = "🏆 <b>Рейтинг мультивселенной</b>\n"
    text += block("⚔️ Топ боевой мощи", top_power, lambda x: battle_power_label(x[1]))
    ...
    text += "\n<b>🏰 Топ кланов</b>\n"
    if clans:
        for i,c in enumerate(clans,1): text += f"{i}. {e(c.get('name','Клан'))} — PTS {int(c.get('points',0) or 0)} · ур. {int(c.get('level',1) or 1)}\n"
    else:
        text += "пока пусто\n"
    await message.answer(text, reply_markup=back_menu(), parse_mode="HTML")
...
async def draw_card_cb(callback: types.CallbackQuery):
    ...
    if wait > 0 and paid <= 0 and not is_owner(callback.from_user.id):
        await callback.message.answer(
            f"🎴 <b>{e(p.get('name', callback.from_user.full_name))}</b>\n\n"
            f"Следующая бесплатная попытка будет доступна через <b>{e(format_wait_hms(wait))}</b>.\n"
            "Можно купить попытки в магазине или получить их через крафт.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [button(text="🎴 Купить попытки", callback_data="shop_attempts")],
                [button(text="⚒️ Крафт попыток", callback_data="craft")],
                [button(text="⬅️ Меню", callback_data="menu")],
            ]),
            parse_mode="HTML"
        )
        await callback.answer("Попытка ещё не готова.", show_alert=True)
        return
    if paid > 0 and wait > 0 and not is_owner(callback.from_user.id):
        p["card_attempts"] = paid - 1
        source = "покупная попытка"
    else:
        p["last_free_pack"] = datetime.now().isoformat()
        p["free_pack_notified"] = False
        source = "бесплатная попытка"
    card = roll_card(weights=FREE_PACK_WEIGHTS, universe_id=selected_universe_id(p))
    result = add_card(p, card["id"])
    add_xp(p, 80)
    add_pass_task_progress(p, "chest", 1)
    ...
    mark_data_dirty("data_changed")
    await callback.message.answer(
        f"🎴 <b>Карта получена</b>\n"
        f"Источник: <i>{e(source)}</i>\n\n"
        f"{CE['collection']} <b>{e(card['name'])}</b> — {rarity_label(card['rarity'])}\n"
        f"🌌 {e(card.get('universe_name') or card.get('anime', ''))}\n"
        f"{e(result)}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [button(text="🎴 Получить ещё", callback_data="draw_card")],
            [button(text="🃏 Коллекция", callback_data="collection:page:0"), button(text="⬅️ Меню", callback_data="menu")],
        ]),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def craft_attempts_cb(callback: types.CallbackQuery):
    ...
    mark_data_dirty("data_changed")
    await callback.message.answer(f"⚒️ Готово: +<b>{made}</b> попыток получить карту.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="🎴 Получить карту", callback_data="draw_card")],[button(text="⬅️ Крафт", callback_data="craft")]]), parse_mode="HTML")
...
async def buy_attempts_cb(callback: types.CallbackQuery):
    ...
    mark_data_dirty("data_changed")
    await callback.message.answer(f"✅ Куплено: +<b>{amount}</b> попыток.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="🎴 Получить карту", callback_data="draw_card")],[button(text="⬅️ Магазин", callback_data="shop")]]), parse_mode="HTML")
...
async def shop_cases_cb(callback: types.CallbackQuery):
    ...
    rows.append([button(text="📜 Возможные награды", callback_data="cases_rewards")])
    rows.append([button(text="⬅️ Магазин", callback_data="shop")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def shop_battlepass_cb(callback: types.CallbackQuery):
    ...
    rows.append([button(text="🎟 Открыть мультипасс", callback_data="multipass")])
    rows.append([button(text="⬅️ Магазин", callback_data="shop")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def buy_battlepass_cb(callback: types.CallbackQuery):
    ...
    mark_data_dirty("data_changed")
    await callback.message.answer("✅ Боевой пропуск активирован.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="🎟 Мультипасс", callback_data="multipass")],[button(text="⬅️ Меню", callback_data="menu")]]))
    await callback.answer()
...
async def premium_info_cb(callback: types.CallbackQuery):
    ...
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="💠 Привилегии", callback_data="privileges")],[button(text="⬅️ Магазин", callback_data="shop")]]), parse_mode="HTML")
    await callback.answer()
...
async def cases_rewards_cb(callback: types.CallbackQuery):
    text=("📜 <b>Награды в кейсах Anime Battle Multiverse</b>\n\n"
          ...
          "<b>Мистик-кейс</b>\n7000–150000 💎, 35–250 попыток, VIP/MVP/X и редкие мистик-карты.")
...
async def send_clan_menu(message, user):
    ...
    if not cid or cid not in clan_store():
        text=(f"🏰 <b>Клан</b>\n\nПривет, <b>{e(p.get('name', user.full_name))}</b>.\nПохоже, у тебя ещё нет клана. Создай свой или присоединись к открытому.")
        ...
        text=(f"🏰 <b>{e(clan.get('name','Клан'))}</b>\n\n"
              f"PTS клана: <b>{int(clan.get('points',0) or 0)}</b>\n"
              f"Уровень: <b>{int(clan.get('level',1) or 1)}</b>\n"
              f"Вход от силы: <b>{int(clan.get('min_power',0) or 0)}</b>\n"
              f"Глава: <b>{e(DATA.get('users',{}).get(str(clan.get('leader','')),{}).get('name','неизвестно'))}</b>\n"
              f"Участники: <code>{e(', '.join(member_names) or 'нет')}</code>")
        rows.append([button(text="👥 Участники", callback_data="clan_members"), button(text="🚪 Покинуть клан", callback_data="clan_leave_ask")])
    rows.append([button(text="⬅️ Меню", callback_data="menu")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def clan_members_cb(callback: types.CallbackQuery):
    ...
    for i,mid in enumerate(clan.get("members",[])[:50],1):
        ...
        lines.append(f"{i}. {e(mp.get('name', mid))} — {battle_power_label(mp)}")
...
async def referral_cb(callback: types.CallbackQuery):
    ...
    text=("🔗 <b>Реферальная программа</b>\n\n"
          f"Твоя ссылка:\n<code>{e(link)}</code>\n\n"
          f"Сегодня пригласил: <b>{int(p.get('ref_today',0) or 0)}</b>\n"
          f"Приглашено всего: <b>{int(p.get('ref_count',0) or 0)}</b>\n"
          f"Заработано попыток: <b>{int(p.get('ref_earned',0) or 0)}</b>\n\n"
          "За каждого активного реферала: +3 попытки.")
    rows=[[button(text="🎁 Вехи", callback_data="ref_claim")],[button(text="⬅️ Меню", callback_data="menu")]]
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def cases(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Кейсы открывают 7 наград и могут давать редкие карты, попытки, фисташки и привилегии.",
        reply_markup=kb,
        parse_mode="HTML"
    )
    await callback.answer()
...
async def successful_payment(message: types.Message):
    ...
    if parsed["kind"] == "multipass":
        ...
        await flush_data_now_async("payment_multipass")
        await message.answer(
    ...
    if parsed["kind"] == "star_pack":
        ...
        await flush_data_now_async("payment_star_pack")
        await message.answer(
...
async def send_admin_panel(message, user):
    ...
    text = (
        ...
        f"🧩 Восстановление записей: источников <b>{recovery.get('sources', 0)}</b>, найдено <b>{recovery.get('after', total_all)}</b>\n\n"
        "Безопасные команды:\n"
        ...
        "<code>/recover_users_patch17</code> — восстановить игроков с учётом Neon\n"
        "<code>/compensate_patch23</code> — новая компенсация текущего обновления"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def send_admin_users(message, page=0):
    ...
    text = (
        f"👥 <b>Живые игроки</b> — страница {page + 1}/{pages}\n\n"
        "Здесь показаны только игроки, которые не заблокировали бота и заходили за последние 30 дней. "
        f"Заблокировали бота: <b>{admin_blocked_count()}</b> · Неактивные 30+ дней: <b>{admin_inactive_count()}</b>.\n"
...
async def send_admin_user(message, uid):
    recover_users_from_all_sources(save=True)
    ensure_admin_known_users()
    ...
    text = (
        ...
        f"Мультипасс: <b>{'premium' if p.get('pass_premium') else p.get('pass_purchase_request', 'нет')}</b> | cap {p.get('pass_premium_cap', 0)} | срок: <code>{e(pass_until_label(p))}</code>\n"
...
async def recover_users_patch16_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="👥 Все игроки", callback_data="admin_users")]])
    )
...
async def sync_neon_patch17_cmd(message: types.Message):
    ...
    recovery = recover_users_from_all_sources(save=False)
    ensure_admin_known_users()
    ...
    await message.answer(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="🧠 Хранилище", callback_data="admin_storage")], [button(text="👥 Все игроки", callback_data="admin_users")]])
    )
...
async def recover_users_patch17_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="👥 Все игроки", callback_data="admin_users")], [button(text="🧠 Хранилище", callback_data="admin_storage")]])
    )
...
@dp.message(Command("compensate_patch23"))
async def compensate_patch23_cmd(message: types.Message):
    if not is_owner(message.from_user.id):
        await message.answer("⛔ Только владелец.")
        return
    ...
    for uid, p in list(DATA.get("users", {}).items()):
        ...
        try:
            await bot.send_message(
                int(uid),
                "🌌 <b>Компенсация за большое RPG-обновление</b>\n\n"
                "Мультивселенная стала больше: выбор аниме, попытки, кланы, магазин, кейсы, профиль и новые коллекции. "
    ...
    mark_data_dirty("data_changed")
    await message.answer(
        "✅ <b>PATCH23 компенсация обработана</b>\n\n"
