...
UNIVERSE_ALIAS_BY_ANIME = {
    ...
    "Кимэцу но Яйба": ("demon_slayer", "Demon Slayer / Клинок демонов"),
    "Kimetsu no Yaiba": ("demon_slayer", "Demon Slayer / Клинок демонов"),
    "Pokemon": ("pokemon", "Pokémon"),
    "Pokémon": ("pokemon", "Pokémon"),
    "Покемон: фильм": ("pokemon", "Pokémon"),
    "Boku no Hero Academia": ("my_hero_academia", "My Hero Academia"),
    "Моя геройская академия": ("my_hero_academia", "My Hero Academia"),
    "Человек-бензопила": ("chainsaw_man", "Chainsaw Man"),
    "Hunter × Hunter": ("hunter_x_hunter", "Hunter x Hunter"),
    "Shingeki no Kyojin": ("attack_on_titan", "Attack on Titan"),
    "Атака на титанов": ("attack_on_titan", "Attack on Titan"),
    "Fate/stay night": ("fate", "Fate"),
    "Fate/Zero": ("fate", "Fate"),
    "Fate/Grand Order": ("fate", "Fate"),
    "JoJo's Bizarre Adventure": ("jojo", "JoJo"),
    "ДжоДжо": ("jojo", "JoJo"),
    "Fairy Tail": ("fairy_tail", "Fairy Tail"),
    "Хвост Феи": ("fairy_tail", "Fairy Tail"),
...
UNIVERSE_PRIORITY = [
    ...
    "one_punch_man", "pokemon", "baki", "demon_slayer", "my_hero_academia",
    "black_clover", "hunter_x_hunter", "attack_on_titan", "chainsaw_man", "fate",
    "jojo", "fairy_tail", "mob_psycho_100", "dr_stone", "fullmetal_alchemist",
    "berserk", "solo_leveling", "tensei_slime", "premium_cosmic",
]

# В меню выбора показываем только большие игровые миры. Маленькие аниме-пулы остаются
# в общем мультивселенном призыве, но больше не портят витрину строками на 2–5 карт.
FEATURED_UNIVERSE_IDS = [
    "naruto_boruto", "one_piece", "dragon_ball", "bleach", "jujutsu_kaisen",
    "one_punch_man", "pokemon", "baki", "demon_slayer", "my_hero_academia",
    "black_clover", "hunter_x_hunter", "attack_on_titan", "chainsaw_man",
    "fate", "jojo", "fairy_tail",
]
...
UNIVERSE_EMOJI = {
    ...
    "jojo": "⭐", "fairy_tail": "🐉", "mob_psycho_100": "🌀", "dr_stone": "🧪",
    "fullmetal_alchemist": "⚙️", "berserk": "🩸", "solo_leveling": "🗡", "tensei_slime": "💧",
    "premium_cosmic": "♾",
}
UNIVERSE_ONBOARDING_VERSION = "patch22_super_fat_universes"
...
def main_menu_text(user=None):
    ...
    if user:
        ...
        universe_line = f"\n🎯 <b>Твой мир:</b> {e(universe_label(selected_universe_id(p)))}\n"
    return (
        f"{CE['start']} <b>Anime Battle Multiverse</b>\n"
        "<i>Разлом открыт. Собери отряд, выбери мир и докажи, что твоя легенда сильнее.</i>\n"
        f"{universe_line}\n"
        "🌌 <b>Вселенная</b> — настрой призывы под любимое аниме.\n"
        "⚔️ <b>Режимы</b> — бои, арены, PvP и рейды между мирами.\n"
        f"{PISTACHIOS_LABEL} — основная валюта.\n"
        f"{DRAGONITE_LABEL} — редкая валюта для особых кейсов.\n\n"
        f"{CE['free_chest']} Бесплатный сундук — каждые 3 часа.\n"
        f"{CE['luffy']} Путь Луфи — личная 10-дневная цепочка."
    )
...
async def send_universe_menu(message, user, page=0, intro=False):
    ...
    title = "🌌 <b>Выбери свою вселенную</b>"
    if intro:
        title = "🌌 <b>Разлом мультивселенной открыт</b>"

    text = (
        f"{title}\n\n"
        "Ты выбираешь мир, из которого чаще будут приходить персонажи в призывах.\n"
        "Бои, арены и рейды остаются общими — любой мир может столкнуться с любым.\n\n"
        f"Сейчас: <b>{e(universe_label(current))}</b>\n"
    )

    rows = []
    all_mark = "✅" if current == "all" else "🌌"
    rows.append([button(text=f"{all_mark} Все мультивселенные", callback_data="universe:set:all")])

    pair = []
    for rec in shown:
        uid = rec["id"]
        mark = "✅" if uid == current else universe_emoji(uid)
        pair.append(button(text=f"{mark} {rec['name'][:24]}", callback_data=f"universe:set:{uid}"))
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:
