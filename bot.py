...
PATCH_VERSION = "PATCH40_FINAL_MULTIVERSE"
DATA_SCHEMA_VERSION = 40
STORAGE_VERSION = "40.0"
...
def main_menu_text(user=None):
    ...
    fistiks = dragonite = power = extra_attempts = owned = total = season_sp = 0
    daily_ready = True
    p = {}
    if user:
        ...
        extra_attempts = max(0, int(p.get("card_attempts", 0) or 0))
        ...
        season_sp = max(0, int(p.get("season_xp", 0) or 0))
    else:
        sinfo = season_info()
    status = draw_status(p) if user else {"detail": "доступен"}
    return (
        ...
        f"🃏 Персонажи: <b>{short_number(owned)}/{short_number(total)}</b>\n"
        f"🎴 Призвать: <b>{e(status['detail'])}</b> · доп. попытки <b>{short_number(extra_attempts)}</b>\n"
        f"{CE['pistachios']} <b>{short_number(fistiks)}</b> · {CE['dragonite']} <b>{short_number(dragonite)}</b>\n"
        f"{CE['season']} Сезон {sinfo['number']}: <b>{short_number(season_sp)} SP</b> · ⏳ {e(season_time_left_label(sinfo))}\n"
...
async def draw_card_to_message(message, user, callback=None):
    ...
    if paid > 0 and wait > 0 and not is_owner(user.id):
        ...
        p["last_free_pack"] = app_now().isoformat()
        p["free_pack_notified"] = False
        source = "бесплатный призыв"

    card, pity_note = roll_card_with_pity(p, weights=FREE_PACK_WEIGHTS, universe_id=selected_universe_id(p))
...
async def case_open(callback: types.CallbackQuery):
    ...
    add_xp(p, 500)
    add_season_xp(p, SEASON_XP_REWARDS["case"])
    mark_data_dirty("case_open")
