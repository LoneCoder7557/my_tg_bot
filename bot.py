...
PATCH_VERSION = "PATCH21_FAT_UNIVERSES"
...
BOT_SHORT_DESCRIPTION = "Anime Battle Multiverse — RPG-карты, выбор аниме-вселенной, арены, рейды, PvP и редкие Absolute-персонажи."
BOT_PUBLIC_DESCRIPTION = (
    "🌌 Anime Battle Multiverse\n"
    "Анимешная RPG-мультивселенная: выбери любимый мир, собирай персонажей, усиливай колоду и сражайся против других вселенных.\n\n"
    "Наруто, One Piece, Dragon Ball, Bleach, Jujutsu Kaisen, One Punch Man, Baki и другие миры получают отдельные призывы. Обычные сундуки дают базовый прогресс, а сверх-Absolute персонажи вынесены в редкие платные кейсы и Stars-наборы."
)
...
PATCH16_COMPENSATION_MOON_COINS = 1
...
# PATCH21: вселенные для призывов + закрытые Super Absolute персонажи.
# Бои/рейды остаются мультивселенными, а сундуки/крафт/кейсы смотрят на выбранный игроком мир.
UNIVERSE_ALIAS_BY_ANIME = {
    ...
    "Baki": ("baki", "Baki / Баки"),
    "Баки": ("baki", "Baki / Баки"),
    "Demon Slayer": ("demon_slayer", "Demon Slayer / Клинок демонов"),
    "Клинок, рассекающий демонов": ("demon_slayer", "Demon Slayer / Клинок демонов"),
    "Chainsaw Man": ("chainsaw_man", "Chainsaw Man / Человек-бензопила"),
    "Человек-бензопила": ("chainsaw_man", "Chainsaw Man / Человек-бензопила"),
    "Покемон": ("pokemon", "Покемон"),
    ...
    "Доктор Стоун": ("dr_stone", "Доктор Стоун"),
    "Dr. Stone": ("dr_stone", "Доктор Стоун"),
    "Fullmetal Alchemist": ("fullmetal_alchemist", "Fullmetal Alchemist"),
    "Dandadan": ("dandadan", "Dandadan"),
    "Mashle": ("mashle", "Mashle"),
    "Overlord": ("overlord", "Overlord"),
    "Re:Zero": ("rezero", "Re:Zero"),
    "Берсерк": ("berserk", "Берсерк"),
    "Berserk": ("berserk", "Берсерк"),
    "Solo Leveling": ("solo_leveling", "Solo Leveling"),
    "Поднятие уровня в одиночку": ("solo_leveling", "Solo Leveling"),
    "Tensei Slime": ("tensei_slime", "Tensei Slime"),
    "О моём перерождении в слизь": ("tensei_slime", "Tensei Slime"),
...
UNIVERSE_PRIORITY = [
    "naruto_boruto", "one_piece", "dragon_ball", "bleach", "jujutsu_kaisen",
    "one_punch_man", "baki", "demon_slayer", "chainsaw_man", "pokemon",
    "my_hero_academia", "black_clover", "hunter_x_hunter", "attack_on_titan", "fate",
    "jojo", "fairy_tail", "mob_psycho_100", "solo_leveling", "tensei_slime", "premium_cosmic",
]

UNIVERSE_EMOJI = {
    "all": "🌌", "naruto_boruto": "🌪", "one_piece": "🏴‍☠️", "dragon_ball": "🟠",
    "bleach": "🗡", "jujutsu_kaisen": "🧿", "one_punch_man": "👊", "baki": "🥊",
    "demon_slayer": "🌊", "chainsaw_man": "🪚", "pokemon": "⚡", "my_hero_academia": "🦸",
    "black_clover": "🍀", "hunter_x_hunter": "🎣", "attack_on_titan": "🧱", "fate": "⚔️",
    "jojo": "⭐", "fairy_tail": "🐉", "mob_psycho_100": "🌀", "solo_leveling": "🗡",
    "tensei_slime": "💧", "premium_cosmic": "♾",
}
UNIVERSE_ONBOARDING_VERSION = "patch21_universe_rpg_restart"
...
def universe_pool_note(rec):
    free_total = int(rec.get("free_total", rec.get("total", 0)) or 0)
    premium_total = int(rec.get("premium_total", 0) or 0)
    if rec.get("id") == "premium_cosmic":
        return f"♾ платный сектор · {premium_total or rec.get('total', 0)} закрытых карт"
    if free_total >= 100:
        return f"✅ большой мир · {free_total}+ карт"
    return f"🛠 растущий мир · {free_total}/100 карт"
        ...
        try:
            if isinstance(event, types.CallbackQuery) and event.message:
                ...
                delete_exact = {
                    ...
                    "battle:start", "online_search", "cases", "events", "universe",
                }
                delete_prefixes = (
                    "pack_info:", "collection:page:", "card:", "battle:arena:", "battle:arena_page:", "battle:diff:",
                    "pvp_source:", "universe:page:", "universe:set:",
                )
...
def main_menu(user_id=None):
    rows = [
        [button(text="🌌 Вселенная", callback_data="universe"), button(text="⚔️ Режимы", callback_data="modes")],
        [button(text="🃏 Коллекция", callback_data="collection:page:0"), button(text="🎁 Награды", callback_data="shop")],
        [button(text="👤 Профиль", callback_data="profile"), button(text="📜 Правила", callback_data="rules")],
    ]
    if user_id:
        player = DATA.get("users", {}).get(str(user_id), {})
...
def main_menu_text(user=None):
    ...
    if user:
        ...
        universe_line = f"\n🎯 <b>Твой призыв:</b> {e(universe_label(selected_universe_id(p)))}\n"
    return (
        f"{CE['start']} <b>Anime Battle Multiverse</b>\n"
        "<i>Разлом миров открыт. Выбери свою вселенную — и собери отряд, который переживёт чужие легенды.</i>\n"
        f"{universe_line}\n"
        "🌌 <b>Суть:</b> сундуки и крафт тянут персонажей из выбранного аниме-мира.\n"
        "⚔️ <b>Бои:</b> арены, PvP и рейды остаются мультивселенными — Наруто может встретить Сайтаму, Блич или Dragon Ball.\n"
        f"{PISTACHIOS_LABEL} — основная валюта прогресса.\n"
        f"{DRAGONITE_LABEL} — редкая премиум-валюта для кейсов и особых покупок.\n\n"
        f"{CE['free_chest']} Бесплатный сундук — каждые 3 часа.\n"
        f"{CE['luffy']} Путь Луфи — личная 10-дневная цепочка без спама."
    )
...
async def set_commands():
    ...
    owner_commands = public_commands + [
        ...
        BotCommand(command="compensate_patch18", description="Компенсация PATCH18"),
    ]
...
async def send_profile(message, user):
    ...
    await message.answer(
        ...
        f"Вселенная призыва: <b>{e(universe_label(selected_universe_id(p)))}</b>\n"
...
async def send_universe_menu(message, user, page=0, intro=False):
    ...
    owned_all, total_all = universe_progress(p, "all")
    title = "🌌 <b>Выбор аниме-вселенной</b>"
    if intro:
        title = "🌌 <b>Новый разлом открыт</b>"
    text = (
        f"{title}\n\n"
        "Выбери мир, из которого будут идти обычные призывы: бесплатный сундук, сундуки за 💎, крафт и часть наград.\n"
        "Арены, PvP и рейды остаются мультивселенными — драться можно против всех.\n\n"
        f"Сейчас выбрано: <b>{e(universe_label(current))}</b>\n"
        f"Коллекция без закрытых Super Absolute: <b>{owned_all}/{total_all}</b>\n\n"
        "♾ Сверх-Absolute персонажи не падают бесплатно. Они доступны только через платные кейсы/Stars/Premium-ивенты.\n"
    )
    rows = []
    all_mark = "✅" if current == "all" else "🌌"
    rows.append([button(text=f"{all_mark} Все мультивселенные · {owned_all}/{total_all}", callback_data="universe:set:all")])
    for rec in shown:
        uid = rec["id"]
        owned, total = universe_progress(p, uid)
        mark = "✅" if uid == current else universe_emoji(uid)
        label = f"{mark} {rec['name'][:25]} · {owned}/{total}"
        if rec.get("id") == "premium_cosmic":
            label = f"♾ Premium Super Absolute · {rec.get('premium_total', rec.get('total', 0))}"
        rows.append([button(text=label, callback_data=f"universe:set:{uid}")])
        text += f"{universe_emoji(uid)} <b>{e(rec['name'])}</b> — {e(universe_pool_note(rec))}\n"
    nav = []
    if page > 0:
...
async def send_shop(message, user):
    ...
    await message.answer(
        ...
        f"{CE['daily_reward']} <b>Ежедневная награда</b> — фисташки, pass XP и редкие бонусы.\n"
        f"{CE['chests']} <b>Сундуки</b> — обычные, усиленные и королевские открытия.\n"
        f"{CE['multipass']} <b>Мультипасс / Донат</b> — Stars, pass, кейсы и уровни.\n"
        f"{CE['rating']} <b>Рейтинг</b> — сила игроков и место в мультивселенной.\n\n"
        f"🎯 <b>Текущий призыв:</b> {e(universe_label(selected_universe_id(p)))}",
...
PATCH18_COMPENSATION_KEY = "PATCH18_FINAL_POLISH"
PATCH18_COMPENSATION_FISTIKS = 1000
PATCH18_COMPENSATION_MOON_COINS = 1
PATCH18_COMPENSATION_PASS_XP = 300


@dp.message(Command("compensate_patch18"))
async def compensate_patch18_cmd(message: types.Message):
    if not is_owner(message.from_user.id):
        await message.answer("⛔ Только владелец.")
        return
    ...
    for uid, p in list(DATA.get("users", {}).items()):
        ...
        if PATCH18_COMPENSATION_KEY in comps:
            continue
        p["fistiks"] = int(p.get("fistiks", 0) or 0) + PATCH18_COMPENSATION_FISTIKS
        p["moon_coins"] = int(p.get("moon_coins", 0) or 0) + PATCH18_COMPENSATION_MOON_COINS
        p["pass_xp"] = int(p.get("pass_xp", 0) or 0) + PATCH18_COMPENSATION_PASS_XP
        comps.append(PATCH18_COMPENSATION_KEY)
        rewarded += 1
        try:
            await bot.send_message(
                int(uid),
                f"🌌 <b>Компенсация PATCH18</b>\n\n"
                "Спасибо, что дождался финальной полировки мультивселенной. "
                "Мы усилили хранилище Neon, старт новичков, pass и инвентарь артефактов.\n\n"
                f"+{PATCH18_COMPENSATION_FISTIKS} 💎 Фисташек\n"
                f"+{PATCH18_COMPENSATION_MOON_COINS} 🐉 Драконита\n"
                f"+{PATCH18_COMPENSATION_PASS_XP} очков мультипасса",
                parse_mode="HTML",
                reply_markup=back_menu(),
            )
            sent += 1
    ...
    mark_data_dirty("data_changed")
    await message.answer(
        "✅ <b>PATCH18 компенсация обработана</b>\n\n"
