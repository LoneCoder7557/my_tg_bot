...
def valid_custom_emoji_id(emoji_id):
    """Return True only for Telegram-sized numeric custom emoji ids.

    PATCH31 hotfix: PATCH30 had several malformed 20-22 digit ids.
    Telegram rejects such ids inside <tg-emoji> and icon_custom_emoji_id, so
    /start and main-menu callbacks could fail silently while polling stayed alive.
    Invalid ids now degrade to ordinary emoji text instead of breaking messages.
    """
    value = str(emoji_id or "").strip()
    return value.isdigit() and 16 <= len(value) <= 19
