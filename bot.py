...
PATCH_VERSION = "PATCH36_RELAUNCH_CORE"
...
PROJECT_HOOK = "Выбери свою аниме-вселенную, собери отряд и докажи, что твой мир сильнее."

BOT_SHORT_DESCRIPTION = "Anime Battle Multiverse — карточная anime-RPG: призывы, коллекции, арены, кланы, рейды и сезоны."
BOT_PUBLIC_DESCRIPTION = (
    "🌌 Anime Battle Multiverse\n"
    "Карточная anime-RPG: выбери вселенную, делай бесплатный призыв раз в 3 часа, собирай команду, усиливай карты, вступай в клан и побеждай в арене, PvP и рейдах.\n\n"
    "Фисташки — основная валюта прогресса. Драконит — редкая премиум-валюта для кейсов, привилегий и особых покупок."
)
...
def _button_icon_key(text: str) -> str | None:
    # Exact matching only for most buttons.
    # Исключение PATCH34: любое слово "назад" получает специальный emoji "Назад".
    t = " ".join(str(text or "").casefold().split())
    # PATCH35: кейсы/сундуки должны показывать обычный видимый значок сундука,
    # а не custom emoji icon_custom_emoji_id.
    if any(word in t for word in ("кейсы", "кейс", "сундуки", "сундук")):
        return None
    # PATCH35: выбор аниме не должен получать декоративные emoji через кнопки.
...
CUSTOM_BUTTON_EMOJI_ENABLED = str(os.getenv("CUSTOM_BUTTON_EMOJI", "0")).strip().lower() in {"1", "true", "yes", "on"}


def button(*args, **kwargs) -> InlineKeyboardButton:
    """Безопасная inline-кнопка.

    По умолчанию используются обычные emoji прямо в тексте: они одинаково
    отображаются на Android, iOS и Desktop. Telegram custom emoji для кнопок
    можно включить переменной CUSTOM_BUTTON_EMOJI=1.
    """
...
ARTIFACTS = [
    {"name": "Печать Рикудо", "text": "стабилизирует чакру, защиту и давление формы", "delta": {"durability": 16, "hax": 12}},
    {"name": "Осколок Хогёку", "text": "помогает пережить критический момент и адаптироваться", "delta": {"durability": 14, "hax": 14}},
    {"name": "Клинок Занпакто", "text": "режет защиту и усиливает дуэльный обмен", "delta": {"power": 16, "speed": 8}},
    {"name": "Сердце Титана", "text": "повышает живучесть под тяжёлым уроном", "delta": {"durability": 24}},
...
# PATCH20 SPEED CLEAN: не сохраняем весь DATA при каждом клике.
# Было: любая кнопка -> JSON/SQLite/Neon save всего прогресса -> лаги 10-15 секунд.
# Стало: изменения помечаются как dirty и сохраняются пачкой в фоне.
...
OWNER_FULL_UNLOCK_VERSION = "PATCH32_OWNER_FAST_GRANTS"
_DATA_DIRTY = False
...
def mark_data_dirty(reason="data_changed"):
    """Пометить DATA изменённым и запланировать фоновое сохранение.

    Функция специально синхронная: её можно вызывать из обычных def и async def.
    Если event loop ещё не запущен, просто ставим dirty — данные будут сохранены при
    ближайшем ручном flush/следующем вызове уже внутри работающего бота.
    """
    global _DATA_DIRTY, _DATA_SAVE_TASK, _DATA_SAVE_LAST_REASON
    _DATA_DIRTY = True
    ...
    if _DATA_SAVE_TASK is None or _DATA_SAVE_TASK.done():
        _DATA_SAVE_TASK = loop.create_task(_delayed_data_save())


def _snapshot_and_save_data_sync():
    """Copy + save DATA outside the asyncio event loop.

    PATCH32 SPEED: copying a large DATA dict inside the event loop can freeze
    every Telegram button for 10-15 seconds. This keeps the polling loop free.
    """
...
async def _delayed_data_save():
    """Debounce-save: один реальный save на пачку быстрых кликов."""
    global _DATA_DIRTY, _DATA_SAVE_LAST_AT, _DATA_SAVE_LAST_ERROR, _DATA_SAVE_COUNTER
    while True:
        await asyncio.sleep(max(1.0, DATA_SAVE_DEBOUNCE_SECONDS))
        if not _DATA_DIRTY:
            return
        ...
        try:
            async with _get_data_save_lock():
                await asyncio.to_thread(_snapshot_and_save_data_sync)
            _DATA_SAVE_LAST_AT = datetime.now().isoformat(timespec="seconds")
            _DATA_SAVE_LAST_ERROR = ""
            _DATA_SAVE_COUNTER += 1
            logger.debug("DATA saved in background: %s", reason)
        except Exception as ex:
            _DATA_SAVE_LAST_ERROR = str(ex)[:300]
            logger.exception("Background DATA save failed: %s", ex)
            _DATA_DIRTY = True


async def flush_data_now_async(reason="manual_flush"):
    """Сохранить DATA прямо сейчас, но без блокировки event loop."""
    global _DATA_DIRTY, _DATA_SAVE_LAST_AT, _DATA_SAVE_LAST_ERROR, _DATA_SAVE_COUNTER, _DATA_SAVE_LAST_REASON
    _DATA_DIRTY = False
    ...
    try:
        ...
        mark_data_dirty("manual_flush_retry")
        return False


def save_data_now(reason="manual_sync"):
    """Редкий синхронный save: использовать только при старте/ручной админ-синхронизации."""
    global _DATA_DIRTY, _DATA_SAVE_LAST_AT, _DATA_SAVE_LAST_ERROR, _DATA_SAVE_COUNTER, _DATA_SAVE_LAST_REASON
    _DATA_DIRTY = False
    ...
    try:
        ...
        mark_data_dirty("manual_sync_retry")
        return False
...
def build_universes(cards):
    data = {}
    for c in cards:
        uid = c.get("universe") or _stable_universe_id(c.get("anime", ""))
...
def universe_progress(player, universe_id):
    ...
    if not uid:
        ...
        return owned, total
    total = sum(1 for c in CARDS if c.get("universe") == uid and not is_super_absolute_card(c))
    owned = 0
    for cid, info in collection.items():
        c = CARD_BY_ID.get(cid)
        if c and c.get("universe") == uid and not is_super_absolute_card(c) and int((info or {}).get("count", 0) or 0) > 0 and bool((info or {}).get("unlocked", True)):
            owned += 1
    return owned, total
...
def card_matches_universe(card, universe_id):
    uid = _effective_universe_id(universe_id)
    return True if not uid else (card or {}).get("universe") == uid
...
def scoped_owned_card_items(player, universe_id=None):
    ...
    if not uid:
        return items
    return [(cid, info) for cid, info in items if CARD_BY_ID.get(cid, {}).get("universe") == uid]
        ...
        try:
            if isinstance(event, types.CallbackQuery) and event.message:
                ...
                delete_exact = {
                    ...
                    "hub:rewards", "hub:more", "custom_universes", "custom_backgrounds",
                }
                delete_prefixes = (
...
class PerUserSerialMiddleware(BaseMiddleware):
    """Последовательно обрабатывает действия одного игрока.

    Telegram может прислать два быстрых нажатия почти одновременно. Без этого
    оба обработчика способны увидеть старый баланс и дважды забрать награду,
    карту, кейс или валюту. Разные игроки по-прежнему обрабатываются параллельно.
    """
    def __init__(self):
        self._locks = {}
    ...
    async def __call__(self, handler, event, data):
        ...
        if not user:
            return await handler(event, data)
        lock = self._locks.setdefault(int(user.id), asyncio.Lock())
        async with lock:
            return await handler(event, data)
...
async def global_error_handler(event: types.ErrorEvent):
    ...
    try:
        if update.callback_query:
            await update.callback_query.answer("Что-то пошло не так. Повтори действие через пару секунд.", show_alert=True)
        elif update.message:
            await update.message.answer(
                "⚠️ Действие не завершилось. Прогресс не должен пропасть — открой 🏠 Меню и повтори.",
                reply_markup=quick_reply_menu(update.message.from_user.id if update.message.from_user else None),
            )
    except Exception:
        pass
    return True
...
def card_public_description(card):
    """Короткое нормальное описание карты без PATCH/ID/emoji/debug-мусора."""
    card = card or {}
    raw = str(card.get("description", "") or "").strip()
    bad_markers = (
        "Карта добавлена", "PATCH", "World System", "Тренировочный режим",
        "emoji=", "emoji =", "media/", "card_id", "ID:", "source/free", "Редиск", "redis"
    )
    if raw:
        clean = re.sub(r"\s+", " ", raw).strip()
        if not any(marker.casefold() in clean.casefold() for marker in bad_markers) and 50 <= len(clean) <= 700:
            return clean
    name = _clean_card_field(card.get("name"), "Персонаж")
    anime = _clean_card_field(card.get("anime") or card.get("universe_name"), "мультивселенной")
    form = _clean_card_field(card.get("form"), "основная форма")
    role = _clean_card_field(card.get("role"), "боец отряда")
    plus = _clean_card_field(card.get("plus"), "сильное давление в ключевой момент")
    minus = _clean_card_field(card.get("minus"), "нуждается в правильной поддержке команды")
    abilities = _clean_card_field(card.get("abilities"), "тактика, скорость и контроль темпа")
    role_l = role.casefold()
    if "танк" in role_l or "защит" in role_l:
        angle = "держит удар, прикрывает союзников и даёт команде время раскрыть сильные карты"
    elif "поддерж" in role_l or "хил" in role_l:
        angle = "усиливает союзников, стабилизирует бой и помогает команде пережить опасный размен"
    elif "контроль" in role_l:
        angle = "ломает темп соперника, мешает его ключевым ходам и открывает окно для атаки"
    elif "ассас" in role_l or "убий" in role_l or "дамаг" in role_l:
        angle = "быстро наказывает слабую цель и опасен, когда получает пространство для атаки"
    else:
        angle = "раскрывается через грамотную позицию, синергию с отрядом и своевременный вход в бой"
    return (
        f"{name} из «{anime}» представлен в форме «{form}». "
        f"В отряде это {role}: {angle}. "
        f"Сильная сторона — {plus}; слабое место — {minus}. "
        f"Ключевые возможности: {abilities}."
    )
...
def _iter_extra_storage_candidates():
    ...
    Это не перезаписывает прогресс пустышкой: ниже используется _merge_users_data(),
    которая сохраняет более сильную версию игрока.
    """
    folders = []
...
def recover_users_from_all_sources(save=True, merge_existing=False):
    """Возвращает только пропавшие записи пользователей из backup/DB.

    По умолчанию существующие аккаунты не объединяются со старыми копиями:
    именно такое объединение раньше возвращало потраченные валюты, снятые баны
    и истёкший Premium. merge_existing оставлен только для ручной диагностики.
    """
    DATA.setdefault("users", {})
    before_users = copy.deepcopy(DATA.get("users", {}) or {})
    source_count = 0
    added_ids = []
    for _path, candidate in _iter_extra_storage_candidates():
        source_count += 1
    ...
    changed = DATA.get("users", {}) != before_users
    if save and changed:
        mark_data_dirty("recover_missing_users")
    return {
        "sources": source_count,
        "before": len(before_users),
...
def grant_manual_pass(uid, days, granted_by=None):
    ...
    p["pass_granted_at"] = datetime.now().isoformat()
...
def take_manual_pass(uid, removed_by=None):
    ...
    p["pass_until"] = ""
    p["pass_removed_at"] = datetime.now().isoformat()
...
def get_user_data(user):
    ...
    if uid not in users:
        ...
        starter_ids = random_starter_card_ids(STARTER_CARD_COUNT)
        users[uid]["starter_cards"] = starter_ids
        for sid in starter_ids:
            users[uid]["collection"][sid] = {"count": 1, "shards": 0, "level": 1, "unlocked": True}
        changed = True
    ...
    if player.get("collection_normalized_version") != PATCH_VERSION:
        normalize_collection(player)
        player["collection_normalized_version"] = PATCH_VERSION
        changed = True
...
def quick_reply_menu(user_id=None):
    # Четыре действия, которые реально нужны постоянно. Остальное — в inline-меню.
    rows = [
        [types.KeyboardButton(text="🎴 Призыв"), types.KeyboardButton(text="🃏 Коллекция")],
...
def main_menu(user_id=None):
    player = DATA.get("users", {}).get(str(user_id), {}) if user_id else {}
    wait = free_card_wait_minutes(player) if player else 0
    paid = int(player.get("card_attempts", 0) or 0) if player else 0
    summon_state = "ГОТОВ" if wait <= 0 or paid > 0 else compact_wait_label(wait)
    rows = [
        [button(text=f"🎴 Призыв • {summon_state}", callback_data="draw_card")],
        [button(text="🃏 Коллекция", callback_data="collection:home"), button(text="⚔️ Битвы", callback_data="modes")],
        [button(text="🎪 События", callback_data="events"), button(text="🎁 Награды", callback_data="hub:rewards")],
        [button(text="👤 Профиль", callback_data="profile"), button(text="✨ Ещё", callback_data="hub:more")],
    ]
    if user_id:
        if not player.get("luffy_finished") and int(player.get("luffy_day", 0) or 0) < len(LUFFY_PATH_CARDS):
            rows.insert(3, [button(text="🔥 Путь Луфи", callback_data="luffy_path")])
        if is_owner(user_id):
...
def main_menu_text(user=None):
    name = "Игрок"
    universe = "🌌 Все мультивселенные"
    clan = "Нет клана"
    fistiks = dragonite = power = attempts = 0
    wait = 0
    owned = total = 0
    role = "Игрок"
    daily_ready = True
    if user:
        ...
        wait = free_card_wait_minutes(p)
        fistiks = int(p.get("fistiks", 0) or 0)
        dragonite = int(p.get("moon_coins", 0) or 0)
        power = battle_power_label(p)
        owned, total = universe_progress(p, uid)
        daily_ready = p.get("last_daily") != date.today().isoformat()
        role = "Создатель" if is_owner(user.id) else ("Правая рука" if is_right_hand(user.id) else player_title(p))

    summon_status = "готов" if wait <= 0 else f"через {compact_wait_label(wait)}"
    daily_status = "доступна" if daily_ready else "получена"
    return (
        f"{CE['start']} <b>ANIME BATTLE MULTIVERSE</b>\n"
        f"<i>Собери команду. Усиль карты. Забери вершину сезона.</i>\n\n"
        f"<blockquote>"
        f"👤 <b>{e(name)}</b> · {e(role)}\n"
        f"{e(universe)}\n"
        f"🃏 {progress_bar(owned, total)} <b>{short_number(owned)}/{short_number(total)}</b>\n"
        f"⚔️ Сила <b>{short_number(power)}</b> · 🎴 попыток <b>{short_number(attempts)}</b>\n"
        f"💎 <b>{short_number(fistiks)}</b> · 🐉 <b>{short_number(dragonite)}</b>\n"
        f"🎁 Ежедневная: <b>{daily_status}</b> · Призыв: <b>{summon_status}</b>"
        f"</blockquote>\n"
        f"🏰 Клан: <b>{e(clan)}</b>"
    )
...
async def send_rewards_hub(message, user):
    ...
    text = (
        ...
        f"<blockquote>🎴 Бесплатный призыв: <b>{'готов' if wait <= 0 else 'через ' + compact_wait_label(wait)}</b>\n"
        f"🎁 Ежедневная награда: <b>{'доступна' if daily_ready else 'получена'}</b>\n"
        f"🎟 MultiPass: <b>{pass_level}/100</b> уровень\n"
        f"🧰 Кейсы в инвентаре: <b>{sum(int(v or 0) for v in p.get('case_inventory', {}).values())}</b></blockquote>\n"
        "Здесь собраны все регулярные награды — без поиска по десяти меню."
    )
    rows = [
        [button(text="🎴 Забрать призыв", callback_data="draw_card"), button(text="🎁 Ежедневная", callback_data="daily")],
        [button(text="🎟 MultiPass", callback_data="multipass"), button(text="🧰 Кейсы", callback_data="cases")],
        [button(text="🔥 Путь Луфи", callback_data="luffy_path"), button(text="🎟 Промокод", callback_data="promo_help")],
...
async def send_more_hub(message, user):
    ...
    text = (
        "✨ <b>ВСЕ РАЗДЕЛЫ</b>\n\n"
        f"Вселенная: <b>{e(universe_label(selected_universe_id(p)))}</b>\n"
        "Основной экран оставлен коротким, а все дополнительные системы собраны здесь."
    )
    rows = [
        [button(text="⚒️ Крафт", callback_data="craft"), button(text="🏰 Клан", callback_data="clan")],
        [button(text="🏪 Магазин", callback_data="shop"), button(text="🏆 Рейтинг", callback_data="rating")],
        [button(text="👥 Друзья", callback_data="friends"), button(text="🔗 Рефералка", callback_data="referral")],
        [button(text="🌌 Сменить вселенную", callback_data="universe")],
        [button(text="📜 Правила", callback_data="rules"), button(text="👤 Профиль", callback_data="profile")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
def free_card_wait_minutes(player):
    ...
    if not last:
        return 0
    try:
        wait_until = datetime.fromisoformat(last) + timedelta(hours=FREE_CARD_WAIT_HOURS)
    except Exception:
        return 0
...
def make_clan_id(name):
    base = hashlib.md5((name + datetime.now().isoformat()).encode("utf-8")).hexdigest()[:10]
    return "clan_" + base
...
def create_default_clan_for_user(user, player, name=None):
    ...
    clan_store()[cid] = {"id": cid, "name": name, "leader": str(user.id), "members": [str(user.id)], "points": 0, "level": 1, "min_power": 0, "open": True, "created_at": datetime.now().isoformat()}
    player["clan_id"] = cid
    mark_data_dirty("data_changed")
...
def public_profile_text(uid, player):
    ensure_rpg_fields(player)
    visible_collection = {cid: v for cid, v in player.get("collection", {}).items() if cid in CARD_BY_ID}
    ...
    return (
        ...
        f"Титул: <b>{e(player_title(player))}</b>\n"
        f"Вселенная: <b>{e(universe_label(selected_universe_id(player)))}</b>\n"
        f"Клан: <b>{e(clan_name_for(player))}</b>\n"
        f"Коллекция: <b>{unique}/{len(CARDS)}</b> · всего карт <b>{total}</b>\n"
        f"Боевая мощь: <b>{battle_power_display(player)}</b>\n"
        f"Победы: <b>{player.get('wins', 0)}</b> · Поражения: <b>{player.get('losses', 0)}</b>\n"
        f"Premium: <b>{'активен' if player.get('pass_premium') or player.get('premium') else 'неактивен'}</b>"
    )
...
def shop_menu():
    rows = [
        ...
        [button(text="🧰 Кейсы", callback_data="shop_cases"), button(text="🎟 MultiPass", callback_data="multipass")],
...
def make_ui_banner(kind="main"):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / f"{kind}_banner.png"
    ...
    draw.text((75, 340), "COLLECT  •  EVOLVE  •  DOMINATE", font=font_sub, fill=(216, 205, 245))
    draw.rounded_rectangle((72, 425, 565, 486), radius=28, fill=(255, 255, 255), outline=(255, 150, 210), width=2)
    draw.text((105, 439), "SEASON: MULTIVERSE RIFT", font=font_tag, fill=(52, 14, 60))
...
async def send_card_result(message, card_id, caption, reply_markup=None):
    ...
    p = media_path(card_id) or make_card_banner(card_id)
    if p:
        f = FSInputFile(p)
        ext = p.suffix.lower()
        try:
...
def pull_pack_reward(player, weights, exclude=None, universe_id=None, allow_super_absolute=False):
    card, pity_note = roll_card_with_pity(player, weights=weights, exclude=exclude, universe_id=universe_id, allow_super_absolute=allow_super_absolute)
    ...
    def _pool(ignore_exclude=False, ignore_rarity=False, ignore_universe=False, allow_paid=False):
        ...
        for c in CARDS:
            ...
            if not ignore_rarity and allowed_rarities is not None and c["rarity"] not in allowed_rarities:
                continue
            if uid and not ignore_universe and c.get("universe") != uid:
                continue
...
async def start(message: types.Message):
    ...
    first_relaunch_view = not bool(p.get("patch36_dashboard_seen"))
    p["patch36_dashboard_seen"] = True
    ...
    # На первом входе показываем только выбор мира, а не четыре окна подряд.
    if p.get("universe_onboarding_seen") != UNIVERSE_ONBOARDING_VERSION:
        await send_universe_menu(message, message.from_user, 0, intro=True)
...
async def send_profile(message, user):
    ...
    scoped_items = scoped_owned_card_items(p, scope_uid)
    total = sum(int(info.get("count", 0) or 0) for _, info in scoped_items)
    unique = len(scoped_items)
    _, scope_total = universe_progress(p, scope_uid)
    role = OWNER_LABEL if is_owner(user.id) else ("🤝 Правая рука" if is_right_hand(user.id) else "Игрок")
    pass_state = "активен" if p.get("pass_premium") or p.get("premium") else "не активен"
    await message.answer(
        f"{CE['profile']} <b>Профиль</b>\n\n"
        f"{CE['user_name']} <b>{e(p.get('name', user.full_name))}</b> · {e(role)} · ID <code>{user.id}</code>\n"
        f"🏷 Титул: <b>{e(player_title(p))}</b>\n"
        f"{CE['universe_selected']} Вселенная: <b>{e(universe_label(scope_uid))}</b>\n"
        f"{CE['clan']} Клан: <b>{e(clan_name_for(p))}</b>\n"
        f"{CE['collection']} Коллекция: <b>{short_number(unique)}/{short_number(scope_total)}</b> · всего <b>{short_number(total)}</b>\n"
        f"⚔️ Сила: <b>{battle_power_display(p)}</b>\n"
        f"🎴 Попытки: <b>{short_number(available_attempts(p))}</b>\n\n"
        f"{MULTIPASS_LABEL}: <b>{pass_state}</b>\n"
        f"🥊 Победы/поражения: <b>{short_number(p.get('wins', 0))}/{short_number(p.get('losses', 0))}</b>",
...
async def send_shop(message, user):
    ...
    await message.answer(
        ...
        "Выбери раздел ниже: попытки, кейсы, MultiPass, премиум или обмен.",
...
async def send_chests(message, user):
    p = get_user_data(user)
    rows = [
        [button(text="🎴 Бесплатный призыв", callback_data="pack_info:free")],
...
async def buy_pack(callback: types.CallbackQuery):
    ...
    if kind == "free":
        ...
        if last:
            ...
            if now < wait_until:
                ...
                await callback.answer(f"Бесплатный призыв будет доступен через {mins} мин.", show_alert=True)
                return
        p["last_free_pack"] = now.isoformat()
        p["free_pack_notified"] = False
        count, cost, weights, name, xp = 1, 0, FREE_PACK_WEIGHTS, "Бесплатный призыв", 60
    else:
        pack = SHOP_PACKS[kind]
    ...
    for _ in range(count):
        if kind == "free":
            card = roll_card(weights=weights, exclude=pulled, universe_id=universe_id)
            result = add_card(p, card["id"])
        else:
            card, result = pull_pack_reward(p, weights, exclude=pulled, universe_id=universe_id)
        pulled.add(card["id"])
...
async def send_collection_home(message, user):
    ...
    for pos, (cid, info) in enumerate(strongest[:3], 1):
        c = CARD_BY_ID[cid]
        top_lines.append(f"{pos}. {rarity_label_for_card(c)} <b>{e(c['name'])}</b> · {short_number(card_power(c, int(info.get('level',1) or 1)))}")
    if not top_lines:
        top_lines = ["Коллекция пока пуста."]
    text = (
        ...
        f"💎 {short_number(p.get('fistiks', 0))} · 🐉 {short_number(p.get('moon_coins', 0))}</blockquote>\n"
        f"<b>Сильнейшие карты</b>\n" + "\n".join(top_lines) + "\n\n"
        f"Ивентовые: <b>{short_number(got['event'])}/{short_number(totals['event'])}</b> · "
        f"Super Absolute: <b>{short_number(got['super'])}/{short_number(totals['super'])}</b>\n"
        f"Мистик: <b>{short_number(got['mystic'])}/{short_number(totals['mystic'])}</b> · "
        f"Артефакты: <b>{short_number(art_have)}/{short_number(art_total)}</b>"
    )
    rows = [
        [button(text="📚 Все карты", callback_data="collection:page:0:all:power")],
        [button(text="⚪ По редкости", callback_data="collection:ordinary"), button(text="🧿 Артефакты", callback_data="artifacts:page:0")],
        [button(text="🎪 Ивентовые", callback_data="collection:filter:event"), button(text="⚫ Super Absolute", callback_data="collection:filter:super")],
        [button(text="🔮 Мистик", callback_data="collection:filter:mystic"), button(text="⚒️ Крафт", callback_data="craft")],
        [button(text="🌌 Сменить мир", callback_data="universe"), button(text="⬅️ Меню", callback_data="menu")],
    ]
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def send_collection(message, user, page=0, rarity_filter="all", sort_mode="power"):
    ...
    scoped_opened_count = len(items)
    _, scoped_total_count = universe_progress(p, scope_uid)
    ...
    if not items:
        ...
        await message.answer(
            ...
            f"{CE['universe_selected']} Вселенная: <b>{e(scope_label)}</b>\n"
            f"Открыто: <b>{short_number(scoped_opened_count)}/{short_number(scoped_total_count)}</b>\n"
            "Карт по этому фильтру пока нет.\n"
            f"Артефактов в инвентаре: <b>{short_number(art_count)}</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
            parse_mode="HTML"
        )
        return
    ...
    for cid, info in items[page * per_page:(page + 1) * per_page]:
        c = CARD_BY_ID[cid]
        lvl = int(info.get('level', 1))
        btn_icon = "⚫" if is_super_absolute_card(c) else RARITY_EMOJI.get(c['rarity'], '⚪')
        text_cards.append("• " + collection_card_line(cid, info))
        rows.append([button(text=f"{btn_icon} {c['name'][:28]} · ур.{lvl} · {short_number(card_power(c,lvl))}", callback_data=f"card:{card_cb_id(cid)}")])
    nav = []
    if page > 0:
    ...
    rows.append([button(text="Назад к разделам", callback_data="collection:home")])
    rows.append([
    ...
    rows.append([button(text="Артефакты", callback_data="artifacts:page:0")])
    rows.append([button(text="Меню", callback_data="menu")])
    title, body = SECTION_HINTS["collection"]
    text = (
        f"{ui_box(title, body)}\n\n"
        f"{CE['universe_selected']} Вселенная: <b>{e(scope_label)}</b>\n"
        f"Фильтр: <b>{e(collection_filter_name(rarity_filter))}</b> · сортировка: <b>{e(collection_filter_name(sort_mode))}</b>\n"
        f"Открыто: <b>{short_number(scoped_opened_count)}/{short_number(scoped_total_count)}</b> · артефактов: <b>{short_number(art_count)}</b>\n\n"
        + "\n".join(text_cards) +
...
async def send_artifacts_collection(message, user, page=0):
    ...
    pages = max(1, (len(items) + per_page - 1) // per_page)
    page = max(0, min(int(page or 0), pages - 1))
    total_count = sum(int(info.get("count", 0) or 0) for _, info in items)
    lines = ["🧿 <b>Коллекция артефактов</b>\n", f"Артефакты усиливают бои, рейды и авто-колоды. В наличии: <b>{len(items)}</b> видов, <b>{total_count}</b> шт.\n"]
    rows = []
    if not items:
...
async def card_detail(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"🎴 <b>Редкость:</b> {rarity_label_for_card(c)}\n"
        f"{'🛑 <b>Класс:</b> Super Absolute\n' if (is_super_absolute_card(c) or c.get('special_tier') == 'super_absolute') else ''}"
        f"📈 <b>Уровень:</b> {level}/{MAX_LEVEL}\n"
        f"⚔️ <b>Сила:</b> {power}\n"
        f"❤️ <b>HP:</b> {hp}\n"
        f"🎯 <b>Роль:</b> {e(c.get('role',''))}\n"
        f"🔝 <b>Предел:</b> {MAX_LEVEL} уровень\n\n"
        f"🟨 <b>Описание</b>\n<blockquote>{e(card_public_description(c))}</blockquote>\n"
        f"📌 {e(next_text)}"
    )
    rows = []
    if cost is not None:
        rows.append([button(text=f"⬆️ Улучшить до {level+1}", callback_data=f"upgrade:{card_cb_id(cid)}")])
    rows.append([button(text="В команду", callback_data=f"deck_add:{card_cb_id(cid)}"), button(text="Назад", callback_data="collection:home")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def apply_promo(message, code):
    ...
    try:
        ...
        max_uses = 0
    if max_uses > 0 and used_count >= max_uses and not is_owner(message.from_user.id):
        await message.answer("Лимит активаций этого промокода уже исчерпан.")
        return
    ...
    await message.answer("\n\n".join(lines), reply_markup=main_menu(message.from_user.id), parse_mode="HTML")
...
async def friend_sendgift_cb(callback: types.CallbackQuery):
    ...
    gifts[fid]=datetime.now().isoformat()
    mark_data_dirty("data_changed")
...
def apply_referral_once(inviter_id, newcomer_id, newcomer_player):
    ...
    add_newbie_task_progress(inviter, "referral", 1)
    add_xp(inviter, 150)
...
async def draw_card_to_message(message, user, callback=None):
    ...
    ensure_rpg_fields(p)
    wait = free_card_wait_minutes(p)
    paid = int(p.get("card_attempts", 0) or 0)
    if wait > 0 and paid <= 0 and not is_owner(user.id):
        kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        await message.answer(
            ...
            f"Покупных попыток: <b>{paid}</b>.\n\n"
            "Пока можно забрать ежедневную награду, пройти событие или усилить колоду.",
            reply_markup=kb,
            parse_mode="HTML",
        )
        if callback:
    ...
    if paid > 0 and wait > 0 and not is_owner(user.id):
        p["card_attempts"] = paid - 1
        source = "дополнительная попытка"
    else:
        p["last_free_pack"] = datetime.now().isoformat()
        p["free_pack_notified"] = False
        source = "бесплатный призыв"

    card, pity_note = roll_card_with_pity(
        p,
        weights=FREE_PACK_WEIGHTS,
        universe_id=selected_universe_id(p),
    )
    result = add_card(p, card["id"])
    add_xp(p, 80)
    ...
    info = p.get("collection", {}).get(card["id"], {})
    level = int(info.get("level", 1) or 1)
    power = card_power(card, level)
    owned, total = universe_progress(p, selected_universe_id(p))
    special = "\n⚫ <b>Класс:</b> Super Absolute" if is_super_absolute_card(card) or card.get("special_tier") == "super_absolute" else ""
    is_new = str(result).startswith("🆕")
    drop_line = "✨ <b>НОВАЯ КАРТА</b>" if is_new else "♻️ <b>ДУБЛИКАТ → ФРАГМЕНТЫ</b>"
    result_clean = re.sub(r"^[🆕♻️]\s*", "", str(result))
    next_wait = free_card_wait_minutes(p)

    caption = (
        f"{drop_line}\n"
        f"<b>{e(card['name'])}</b>\n"
        f"{rarity_label_for_card(card)}{special}\n\n"
        f"🎞 {e(card.get('universe_name') or card.get('anime', ''))}\n"
        f"🎭 {e(card.get('form') or 'Базовая форма')}\n"
        f"⚔️ Сила: <b>{short_number(power)}</b> · уровень <b>{level}</b>\n"
        f"📦 {e(result_clean)}{pity_note}\n\n"
        f"<blockquote>{e(card_public_description(card))}</blockquote>\n"
        f"🃏 Коллекция мира: <b>{short_number(owned)}/{short_number(total)}</b> · источник: {e(source)}\n"
        f"⏳ Следующий бесплатный: <b>{compact_wait_label(next_wait)}</b>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [button(text="🔁 Призвать ещё", callback_data="draw_card")],
        [button(text="🃏 Открыть карту", callback_data=f"card:{card_cb_id(card['id'])}"), button(text="⚔️ В колоду", callback_data=f"deck_add:{card_cb_id(card['id'])}")],
        [button(text="🎁 Награды", callback_data="hub:rewards"), button(text="⬅️ Меню", callback_data="menu")],
    ])
    await send_card_result(message, card["id"], caption, kb)
    if callback:
        await callback.answer("Новая карта!" if is_new else "Дубликат превращён во фрагменты.")
...
async def shop_cases_cb(callback: types.CallbackQuery):
    rows=[]
    text="🧰 <b>Покупка кейсов</b>\n\nКейсы хранятся на балансе и открываются в разделе «Кейсы».\n\n"
    for code,name,cost in CASE_SHOP_PACKS:
        text += f"• {name} — {cost} 🐉\n"
        rows.append([button(text=f"🐉 {cost} → {name}", callback_data=f"buy_case_item:{code}:{cost}")])
    rows.append([button(text="⬅️ Магазин", callback_data="shop")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def buy_case_item_cb(callback: types.CallbackQuery):
    ...
    if int(p.get("moon_coins",0) or 0) < cost and not is_owner(callback.from_user.id):
        await callback.answer("Не хватает драгонита.", show_alert=True); return
    if not is_owner(callback.from_user.id): p["moon_coins"] = int(p.get("moon_coins",0) or 0)-cost
    inv=p.setdefault("case_inventory", {"light":0,"event":0,"holiday":0,"mystic":0})
    inv[code]=int(inv.get(code,0) or 0)+1
    mark_data_dirty("data_changed")
    await callback.message.answer("✅ Кейс добавлен на баланс.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="🧰 Открыть кейсы", callback_data="cases")],[button(text="⬅️ Магазин", callback_data="shop")]]))
    await callback.answer()
...
async def cases_rewards_cb(callback: types.CallbackQuery):
    text=("📜 <b>Награды в кейсах Anime Battle Multiverse</b>\n\n"
          "<b>Лайт-кейс</b>\n300–5000 💎, 3–25 попыток, шанс VIP/карты.\n\n"
          "<b>Ивент-кейс</b>\n3000–50000 💎, 10–100 попыток, редкие карты и ивентовые награды.\n\n"
          "<b>Праздничный кейс</b>\n3000–250000 💎, 10–500 попыток, временная ивентовая валюта и редкие карты.\n\n"
          "<b>Мистик-кейс</b>\n7000–150000 💎, 35–250 попыток, VIP/Elite/Mythic-ранги и редкие мистик-карты.")
...
async def clan_join_list_cb(callback: types.CallbackQuery):
    ...
    if not clans:
        ...
        cid="clan_open_rift"
        clan_store().setdefault(cid, {"id":cid,"name":"Гильдия разлома","leader":"","members":[],"points":0,"level":1,"min_power":0,"open":True,"created_at":datetime.now().isoformat()})
        clans=open_public_clans(10)
...
async def cases(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [button(text=f"💡 Лайт-кейс x{int(inv.get('light',0) or 0)}", callback_data="case_open:light")],
        [button(text=f"⚡ Ивент-кейс x{int(inv.get('event',0) or 0)}", callback_data="case_open:event")],
        [button(text=f"🎉 Праздничный x{int(inv.get('holiday',0) or 0)}", callback_data="case_open:holiday")],
        [button(text=f"🔴 Мистик x{int(inv.get('mystic',0) or 0)}", callback_data="case_open:mystic")],
        [button(text="📜 Возможные награды", callback_data="cases_rewards")],
        [button(text="🏪 Купить кейсы", callback_data="shop_cases"), button(text="⬅️ Меню", callback_data="menu")],
    ])
    await callback.message.answer(
        f"🧰 <b>Кейсы</b>\n\n"
        f"Баланс: <b>{short_number(balance)}</b> 🐉\n"
        f"Текущий призыв: <b>{e(universe_label(selected_universe_id(p)))}</b>\n\n"
        "7 наград внутри: карты, попытки, фисташки и редкие бонусы.",
        reply_markup=kb,
        parse_mode="HTML"
    )
    await callback.answer()
...
async def case_open(callback: types.CallbackQuery):
    ...
    inv = p.setdefault("case_inventory", {"light": 0, "event": 0, "holiday": 0, "mystic": 0})
    if kind == "light":
        weights = {"Обычный": 500, "Редкий": 330, "Эпический": 135, "Легендарный": 30, "Мифический": 5}
    else:
        weights = CASE_WEIGHTS.get(kind, RARITY_WEIGHTS)
    if int(inv.get(kind, 0) or 0) > 0:
        inv[kind] = int(inv.get(kind, 0) or 0) - 1
    else:
        cost = int(CASE_PRICES.get(kind, 8))
        if int(p.get("moon_coins", 0)) < cost and not is_owner(callback.from_user.id):
            await callback.answer(f"Нет кейса и не хватает драгонита. Нужно {cost} 🐉.", show_alert=True)
            return
        if not is_owner(callback.from_user.id):
    ...
    for _ in range(7):
        if random.random() < 0.22:
            attempts = random.choice([3, 5, 10, 25])
            p["card_attempts"] = int(p.get("card_attempts", 0) or 0) + attempts
            got.append(({"name": f"+{attempts} попыток", "rarity": "Редкий", "id": "attempts", "universe_name": "Бонус"}, "Попытки добавлены на баланс."))
            continue
        card, pity_note = roll_card_with_pity(p, weights=weights, exclude=pulled, universe_id=universe_id, allow_super_absolute=True)
        pulled.add(card["id"])
...
async def successful_payment(message: types.Message):
    ...
    if parsed["kind"] == "multipass":
        ...
        saved = await flush_data_now_async("payment_multipass")
        await message.answer(
            "✅ <b>Оплата получена — Premium MultiPass открыт.</b>\n\n"
            "Доступ активирован автоматически до 100 уровня текущей шкалы наград."
            + ("" if saved else "\n\n⚠️ Облачное сохранение временно недоступно; повторная запись уже поставлена в очередь."),
            reply_markup=back_menu(),
            parse_mode="HTML",
        )
        await notify_owner_purchase(
            message.from_user,
            "⭐ <b>Premium MultiPass куплен и активирован</b>\n\n"
    ...
    if parsed["kind"] == "star_pack":
        ...
        saved = await flush_data_now_async("payment_star_pack")
        save_note = "" if saved else "\n\n⚠️ Облачное сохранение временно недоступно; повторная запись уже поставлена в очередь."
        await message.answer(
            "✅ <b>Оплата получена. Набор выдан.</b>\n\n" + reward_text + save_note,
            reply_markup=back_menu(),
            parse_mode="HTML",
        )
        await notify_owner_purchase(
...
async def mega_buy(callback: types.CallbackQuery):
    ...
    for _ in range(amount):
        ...
        for _j in range(pack["count"]):
            card, result = pull_pack_reward(p, pack["weights"], exclude=pulled, universe_id=universe_id)
            pulled.add(card["id"])
            got.append((card, result))
    add_xp(p, 60 * amount)
...
async def send_admin_panel(message, user):
    ...
    live_items = admin_live_player_items()
    total_all = len(DATA.get("users", {}) or {})
    live = len(live_items)
    blocked = admin_blocked_count()
    ...
    text = (
        ...
        "<code>/recover_users_patch16</code> — подтянуть игроков из backups/DB\n"
        "<code>/sync_neon_patch17</code> — синхронизировать DATA ↔ Neon\n"
        "<code>/recover_users_patch17</code> — вернуть только пропавшие записи из backup\n"
        "<code>/storage</code> — проверить Neon/JSON/SQLite\n"
        "<code>/flush_data</code> — сохранить текущий DATA прямо сейчас\n"
        "<code>/compensate_patch24</code> — компенсация обновления"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [button(text="🟢 Живые игроки", callback_data="admin_users:live:0"), button(text="📦 Все из базы", callback_data="admin_users:all:0")],
        ...
        [button(text="🧹 Удалить blocked", callback_data="admin_purge_blocked")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ])
    mark_data_dirty("data_changed")
    await message.answer(text, reply_markup=kb, parse_mode="HTML")
...
async def purge_blocked_cmd(message: types.Message):
    ...
    for uid in ids:
        users.pop(uid, None)
    mark_data_dirty("data_changed")
    await message.answer(f"🧹 Удалено записей заблокировавших бота: <b>{len(ids)}</b>.", parse_mode="HTML", reply_markup=back_menu())
...
async def recover_users_patch16_cmd(message: types.Message):
    if not is_owner(message.from_user.id):
        await message.answer("⛔ Только владелец.")
        return
    recovery = recover_users_from_all_sources(save=True)
    ensure_admin_known_users()
...
async def sync_neon_patch17_cmd(message: types.Message):
    ...
    if not DATABASE_URL:
        ...
        return
    before_pg = _count_postgres_users()
    recovery = {"sources": 0}
    ensure_admin_known_users()
    await flush_data_now_async("sync_neon_patch17")
    after_pg = _count_postgres_users()
    await message.answer(
...
async def recover_users_patch17_cmd(message: types.Message):
    ...
    await message.answer(
        ...
        f"Стало записей: <b>{recovery.get('after', 0)}</b>\n"
        f"Добавлено/вернулось: <b>{recovery.get('added', 0)}</b>\n"
        f"Игроков в Neon: <b>{_count_postgres_users()}</b>\n\n"
...
async def admin_purge_blocked_cb(callback: types.CallbackQuery):
    ...
    for uid in ids:
        users.pop(uid, None)
    mark_data_dirty("data_changed")
    await callback.message.answer(f"🧹 Удалено записей blocked: <b>{len(ids)}</b>.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button(text="⬅️ Админ-панель", callback_data="admin")]]))
...
@dp.message(F.text.in_({"🎴 Призыв", "Призыв", "🎴 Получить карту", "Получить карту"}))
...
@dp.message(F.text.in_({"⚔️ Битвы", "Битвы", "⚔ Битвы"}))
...
async def luffy_path_notifier():
    """Раз в день около 10:00 напоминает активным игрокам забрать день Пути Луфи."""
