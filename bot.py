...
async def send_admin_users(message, page=0):
    ...
    for i, (uid, p) in enumerate(chunk, page * per_page + 1):
        ...
        display = str(p.get('name', uid))[:22]
        rows.append([
            InlineKeyboardButton(text=f"💬 {display}", url=f"tg://user?id={uid}"),
            InlineKeyboardButton(text="📊 Карточка", callback_data=f"admin_user:{uid}"),
        ])
    nav = []
    if page > 0:
