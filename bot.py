...
PATCH_VERSION = "PATCH35_CLEAN_UI_COLLECTION_LUFFY"
        ...
        try:
            if isinstance(event, types.CallbackQuery) and event.message:
                ...
                delete_exact = {
                    ...
                    "battle:start", "online_search", "cases", "events", "universe", "clan", "referral",
                }
                delete_prefixes = (
...
def quick_reply_menu(user_id=None):
    # Нижняя всплывающая клавиатура: 4 быстрые команды, как в больших card-ботах.
    rows = [
        [types.KeyboardButton(text="🎴 Получить карту"), types.KeyboardButton(text="🃏 Мои карты")],
        [types.KeyboardButton(text="🏠 Меню"), types.KeyboardButton(text="🎟 MultiPass")],
    ]
...
async def ensure_quick_keyboard(message, user):
    # PATCH35: возвращаем нижние быстрые кнопки. Telegram показывает ReplyKeyboard
    # только вместе с отдельным сообщением, поэтому отправляем короткое уведомление
    # один раз на версию патча.
    player = get_user_data(user)
    if player.get("quick_keyboard_note_seen") == PATCH_VERSION:
        return
    player["quick_keyboard_note_seen"] = PATCH_VERSION
    mark_data_dirty("data_changed")
    try:
        await message.answer("⌨️ Быстрые кнопки снизу включены.", reply_markup=quick_reply_menu(user.id))
    except Exception:
        pass
...
def main_menu(user_id=None):
    rows = [
        [button(text="🎴 Получить карту", callback_data="draw_card"), button(text="🃏 Мои карты", callback_data="collection:home")],
...
def main_menu_text(user=None):
    ...
    if user:
        ...
        role = "👑 Создатель" if is_owner(user.id) else ("🤝 Правая рука" if is_right_hand(user.id) else "🎮 Игрок")
    return (
        f"{CE['start']} <b>Anime Battle Multiverse</b>\n"
        f"{CE['user_name']} <b>{e(name)}</b> · {role}\n"
        f"{CE['universe_selected']} {e(universe)}\n"
        f"{CE['clan']} Клан: <b>{e(clan)}</b>\n"
        f"{CE['draw_card']} Попытки: <b>{short_number(attempts)}</b>\n"
        f"{PISTACHIOS_LABEL}: <b>{short_number(fistiks)}</b>\n"
        f"{DRAGONITE_LABEL}: <b>{short_number(dragonite)}</b>\n"
        f"⚔️ Сила: <b>{short_number(power)}</b>\n\n"
...
async def menu_cmd(message: types.Message):
    get_user_data(message.from_user)
    await message.answer(main_menu_text(message.from_user), reply_markup=main_menu(message.from_user.id), parse_mode="HTML")
    await ensure_quick_keyboard(message, message.from_user)
...
async def send_profile(message, user):
    ...
    await message.answer(
        ...
        f"{CE['user_name']} <b>{e(p.get('name', user.full_name))}</b> · ID <code>{user.id}</code>\n"
...
async def send_universe_menu(message, user, page=0, intro=False):
    ...
    title = f"{CE['start']} <b>Выбери вселенную</b>"
    text = (
        f"{title}\n"
        f"{e(p.get('name', user.full_name))}, коллекционируй карты и сражайся.\n\n"
        f"Аниме: <b>{e(universe_label(current))}</b>"
    )
    rows = []
    rows.append([button(text=("✓ Мультивселенная" if current == "all" else "Мультивселенная"), callback_data="universe:set:all")])
    for rec in shown:
        uid = rec["id"]
        label = f"✓ {rec['name'][:34]}" if uid == current else rec['name'][:34]
        rows.append([button(text=f"Аниме: {label}", callback_data=f"universe:set:{uid}")])
    nav = []
    if page > 0:
...
async def send_collection_home(message, user):
    ...
    text = (
        ...
        f"{DRAGONITE_LABEL}: <b>{short_number(p.get('moon_coins', 0))}</b> · {PISTACHIOS_LABEL}: <b>{short_number(p.get('fistiks', 0))}</b>\n"
        f"Аниме: <b>{e(scope_label)}</b>\n\n"
        "Выбери раздел коллекции. Фрагменты перенесены в крафт.\n\n"
        f"<blockquote>"
        f"Обычные карты: {short_number(ordinary_have)}/{short_number(ordinary_total)}\n"
        f"Ивентовые: {short_number(got['event'])}/{short_number(totals['event'])}\n"
        f"Super Absolute: {short_number(got['super'])}/{short_number(totals['super'])}\n"
        f"Мистик: {short_number(got['mystic'])}/{short_number(totals['mystic'])}\n"
        f"Артефакты: {short_number(art_have)}/{short_number(art_total)}"
        f"</blockquote>"
    )
    rows = [
        [button(text="Обычные карты", callback_data="collection:ordinary")],
        ...
        [button(text="Мистик", callback_data="collection:filter:mystic"), button(text="Артефакты", callback_data="artifacts:page:0")],
...
async def draw_card_to_message(message, user, callback=None):
    ...
    if wait > 0 and paid <= 0 and not is_owner(user.id):
        await message.answer(
            f"🎴 <b>{e(p.get('name', user.full_name))}</b>\n\n"
            f"Следующая бесплатная попытка будет доступна через <b>{e(format_wait_hms(wait))}</b>.\n"
            "Можно купить попытки в магазине или скрафтить их из повторок.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    reward_icon = CE['event_item'] if (is_super_absolute_card(card) or card.get("special_tier") in {"super_absolute", "event", "mystic"} or card.get("rarity") in {"Легендарный", "Мифический"}) else CE['draw_card']
    drop_line = "Новая карта открыта." if str(result).startswith("🆕") else re.sub(r"^♻️\s*", "Повтор: ", str(result))
    await message.answer(
        f"{reward_icon} <b>Карта получена</b>\n\n"
        f"<b>{e(card['name'])}</b> — {rarity_label_for_card(card)}{special}\n"
        f"Аниме: {e(card.get('universe_name') or card.get('anime', ''))}\n"
        f"Форма: {e(card.get('form',''))}\n"
        f"{e(drop_line)}\n\n"
        f"<blockquote>{e(card_public_description(card))}</blockquote>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [button(text="Получить ещё", callback_data="draw_card")],
            [button(text="Мои карты", callback_data="collection:home"), button(text="Меню", callback_data="menu")],
        ]),
        parse_mode="HTML"
    )
    if callback:
...
async def referral_cb(callback: types.CallbackQuery):
    ...
    text = (f"{CE['referral']} <b>Реферальная программа</b>\n\n"
            "Зови друзей в мультивселенную: активный игрок даёт тебе попытки, прогресс и будущие вехи.\n\n"
            f"Твоя ссылка:\n<code>{e(link)}</code>\n\n"
            ...
            "За каждого активного реферала: <b>+3 попытки</b>.")
    rows = [
        [button(text="🎁 Забрать вехи", callback_data="ref_claim")],
        [button(text="🔗 Пригласить друзей", callback_data="friend_link"), button(text="➕ Добавить группу", url=f"https://t.me/{bot_name}?startgroup=true")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def send_admin_panel(message, user):
    ...
    text = (
        ...
        "<code>/compensate_patch24</code> — новая компенсация текущего обновления"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
@dp.message(F.text.in_({"🎴 Получить карту", "Получить карту"}))
async def quick_draw_button(message: types.Message):
    await draw_card_to_message(message, message.from_user)


@dp.message(F.text.in_({"🃏 Мои карты", "Мои карты", "Моя коллекция"}))
...
async def unknown(message: types.Message):
    await message.answer("Напиши /start или нажми 🏠 Меню снизу.", reply_markup=quick_reply_menu(message.from_user.id))
