...
PATCH_VERSION = "PATCH26_SMART_UI_QUICK_MENU"
...
UNIVERSE_ONBOARDING_VERSION = "patch24_polish_rpg_universes"
...
def quick_reply_menu(user_id=None):
    ...
    rows = [
        ...
        [types.KeyboardButton(text="🏠 Меню"), types.KeyboardButton(text="🎟 MetaPass")],
    ]
...
async def ensure_quick_keyboard(message, user):
    ...
    if quick_keyboard_note_seen(p):
        return
    await message.answer(
        "⌨️ <b>Быстрые кнопки включены</b>\n\nСнизу теперь есть 4 команды: карта, мои карты, меню и MetaPass.",
        reply_markup=quick_reply_menu(user.id),
        parse_mode="HTML"
    )
...
def main_menu(user_id=None):
    rows = [
        [button(text="🎴 Получить карту", callback_data="draw_card"), button(text="🃏 Мои карты", callback_data="collection:page:0:all:power")],
...
def profile_menu(user_id=None):
    rows = [
        ...
        [button(text="🌌 Сменить вселенную", callback_data="universe"), button(text="📜 Правила", callback_data="rules")],
        [button(text="🎨 Кастомизация", callback_data="customize"), button(text="📊 Статистика", callback_data="profile_stats")],
...
def shop_menu():
    rows = [
        ...
        [button(text="💠 Ранги поддержки", callback_data="privileges"), button(text="🔁 Обмен", callback_data="exchange")],
        [button(text="⬅️ Меню", callback_data="menu")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def send_profile(message, user):
    ...
    await message.answer(
        f"{CE['profile']} <b>Профиль мультивселенной</b>\n\n"
        ...
        f"Боевая мощь: <b>{battle_power_display(p)}</b>\n"
...
async def send_rules(message):
    text = (
        ...
        "<b>4. Редкости</b>\n"
        "Basic/Origin → Rare → Elite/Epic → Legendary → Absolute → Super Absolute.\n"
...
async def universe_set_cb(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"<b>{e(p.get('name', callback.from_user.full_name))}</b>, теперь твой путь: <b>{e(label)}</b>.\n"
        f"<blockquote>{e(universe_victory_text(uid))}</blockquote>\n\n"
        "Выбор сохранён. Менять вселенную можно в профиле."
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [button(text="🎴 Получить карту", callback_data="draw_card"), button(text="🃏 Мои карты", callback_data="collection:page:0:all:power")],
...
async def send_collection(message, user, page=0, rarity_filter="all", sort_mode="power"):
    ...
    for cid, info in items[page * per_page:(page + 1) * per_page]:
        ...
        rows.append([button(text=f"{RARITY_EMOJI.get(c['rarity'],'⚪')} {c['name'][:28]} · ур.{lvl} · {card_power(c,lvl)}", callback_data=f"card:{card_cb_id(cid)}")])
    nav = []
    if page > 0:
    ...
    rows.append([
        ...
        button(text="⚪ Basic", callback_data="collection:filter:common"),
        button(text="🔷 Advanced", callback_data="collection:filter:rare"),
    ])
    rows.append([
        button(text="🟣 Elite", callback_data="collection:filter:epic"),
        button(text="🟡 Majestic", callback_data="collection:filter:legendary"),
        button(text="🔴 Absolute", callback_data="collection:filter:mythic"),
    ])
    rows.append([button(text="🛑 Super Absolute", callback_data="collection:filter:super")])
    rows.append([
...
async def card_detail(callback: types.CallbackQuery):
    ...
    rows.append([button(text="🎴 В команду", callback_data=f"deck_add:{card_cb_id(cid)}"), button(text="⬅️ Назад", callback_data="collection:page:0:all:power")])
    await callback.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
    await callback.answer()
...
async def send_rating(message):
    ...
    text += block("⚔️ Топ боевой мощи", top_power, lambda x: battle_power_display(x[1]))
    text += block("💎 Топ фисташек", top_fistiks, lambda x: short_number(x[1].get("fistiks", 0)))
    text += block("🌌 Топ мультивселенной", top_multi, lambda x: short_number(x[1].get("wins", 0) * 10 + len(x[1].get("collection", {})) * 3 + x[1].get("xp", 0) // 50))
...
async def draw_card_to_message(message, user, callback=None):
    ...
    await message.answer(
        ...
        f"{CE['collection']} <b>{e(card['name'])}</b> — {rarity_label(card['rarity'])}{special}\n"
        f"🌌 {e(card.get('universe_name') or card.get('anime', ''))}\n"
        f"🎭 {e(card.get('form',''))}\n"
        f"{e(result)}\n\n"
        f"<blockquote>{e(card.get('description','Описание скоро будет обновлено.'))}</blockquote>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [button(text="ℹ️ Описание / улучшение", callback_data=f"card:{card_cb_id(card['id'])}")],
            [button(text="🎴 Получить ещё", callback_data="draw_card")],
            [button(text="🃏 Мои карты", callback_data="collection:page:0:all:power"), button(text="⬅️ Меню", callback_data="menu")],
        ]),
        parse_mode="HTML"
    )
    if callback:
...
async def send_modes(message, user):
    ...
    await message.answer(
        ...
        "Пока игроков мало, быстрый бой чаще ставит тебя против бота. Онлайн остаётся доступным.",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
@dp.message(F.text.in_({"🎟 MetaPass", "MetaPass", "Метапасс", "Мультипасс"}))
...
async def keyboard_cmd(message: types.Message):
    await message.answer("⌨️ Быстрые кнопки обновлены.", reply_markup=quick_reply_menu(message.from_user.id))
