...
# PATCH15.2: custom emoji icons for InlineKeyboardButton.
# Telegram does not render <tg-emoji> HTML inside button text; buttons need icon_custom_emoji_id.
BUTTON_ICON_KEYWORDS = [
    ("dragonite", ("драконит", "🐉")),
    ("pistachios", ("фисташ", "💎")),
    ("owner", ("владел", "админ", "admin", "панель", "компенсац", "👑")),
    ("luffy", ("луфи", "luffy", "ника", "gear 5", "🔥")),
    ("rules", ("правил", "📜")),
    ("profile", ("профил", "статист", "друз", "ник", "уведом", "знаки", "👤", "🔔", "👥", "✏")),
    ("collection", ("коллек", "карта", "карты", "персонаж", "колод", "команд", "слот", "улучш", "форма", "🃏", "🎴", "🧬")),
    ("rewards", ("награ", "сундук", "сундуки", "ежеднев", "магазин", "рейтинг", "мультипасс", "донат", "stars", "star", "кейс", "промокод", "купить", "забрать", "pass", "⭐", "🎁", "🆓", "🏆", "🎟", "🧰", "📦")),
    ("modes", ("режим", "бой", "боя", "арен", "онлайн", "рейд", "ивент", "турнир", "босс", "выбор", "удар", "pvp", "⚔", "🌐", "👹", "🎪", "🤖", "🛡")),
    ("start", ("меню", "назад", "домой", "главн", "🏠", "⬅", "↩")),
]
...
def _button_icon_key(text: str) -> str | None:
    ...
    for key, words in BUTTON_ICON_KEYWORDS:
        if any(w.casefold() in t for w in words):
            return key
    return None
...
async def send_collection(message, user, page=0, rarity_filter="all", sort_mode="power"):
    ...
    for cid, info in items[page * per_page:(page + 1) * per_page]:
        c = CARD_BY_ID[cid]
        lvl = int(info.get('level',1))
        rows.append([button(text=f"{RARITY_EMOJI.get(c['rarity'],'⚪')} {c['name'][:32]} ур.{lvl} | {card_power(c,lvl)}", callback_data=f"card:{cid}")])
    nav = []
    if page > 0:
...
async def send_manual_team_picker(message, user, target="solo", page=0):
    ...
    for cid, info in owned[page*per_page:(page+1)*per_page]:
        ...
        rows.append([button(text=f"{mark} {c['name'][:30]}", callback_data=f"manual_team_add:{target}:{cid}")])
    nav = []
    if page > 0:
