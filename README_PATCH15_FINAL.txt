...
────────────────────────
PATCH15.1 CUSTOM EMOJI / BOTROOT
────────────────────────
Добавлены custom Telegram emoji в HTML-сообщения бота:
- старт / Anime Battle Multiverse
- профиль
- режимы
- коллекция
- награды
...
Важно:
- В кнопках оставлены обычные emoji. Это специально: inline-кнопки Telegram не парсят HTML-тег <tg-emoji> как обычный текст сообщения.
- Custom emoji работают в сообщениях, где включён parse_mode="HTML".
- Если Telegram у какого-то пользователя не покажет custom emoji, останется обычный fallback emoji.

Если ZIP содержит папку BotRoot/:
1. Если в GitHub проект хранится внутри папки BotRoot, замени папку BotRoot целиком.
2. Если Render Root Directory пустой, bot.py должен лежать в корне репозитория. Тогда загружай содержимое BotRoot/, а не саму папку.
3. Если хочешь оставить папку BotRoot в репозитории, поставь в Render Root Directory: BotRoot
